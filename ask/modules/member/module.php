...
function check_exists($condition){
	$SQL = "SELECT M.username,M.name,M.role_id,M.id,E.verify,E.expert,E.star,E.item_count,E.solve_item_count,E.reply_count,E.fav_count,E.last_ask_time,E.last_reply_time FROM `".$this->table_member."` AS `M` LEFT JOIN `".$this->table."` AS `E` ON M.username=E.username WHERE ".$condition;
	//$condition = str_replace("id=","M.id=",$condition);
	if($this->DB_master->fetch_one($SQL)){
		return true;
	}else{
		return false;
	}
 ...
 * param array $data 数据
 */
function verify($condition, $data,$ids){
	//没有数据则先插入会员数据
	foreach($ids as $id){
		if(!$this->DB_master->fetch_one("SELECT id FROM " . $this->table . " WHERE id='$id'")){
			$member = $this->DB_master->fetch_one("SELECT id,username,role_id FROM " . $this->table_member . " WHERE id='$id'");
			$this->DB_master->insert(
					$this->table,
					array('id' => $id,'verify' => 1,'username'=> $member['username'],'role_id'=>$member['role_id']),
 ...
 * param array $data 数据
 */
function set_star($condition, $data,$ids){
	//没有数据则先插入会员数据
	foreach($ids as $id){
		if(!$this->DB_master->fetch_one("SELECT id FROM " . $this->table . " WHERE id='$id'")){
			$member = $this->DB_master->fetch_one("SELECT id,username,role_id FROM " . $this->table_member . " WHERE id='$id'");
...
function label(&$LABEL, &$label, &$var){
	...
	foreach($list as $k => $v){
		...
		$list[$k]['url']='';
		if($option['mbtype']=='expert'){
			$list[$k]['url']=$list[$k]['ask_url']=$ask_url= $this->system->controller."/item-ask?exper=".$v['id'];
