...
class P8_Ask_Member_Controller extends P8_Controller{
	...
	function check_exists($params = array()){
		...
		if(isset($params['id'])) $condition = "M.id='".$params['id']."'";
	...
	function verify(&$POST){
		...
		$this->model->verify(" id IN($allids)",array('verify'=>$POST['verify']),$POST['ids']);
		return $data;
	...
	function set_star(&$POST){
		...
		$this->model->set_star(" id IN($allids)",array('star'=>$POST['star']),$POST['ids']);
		return $data;
