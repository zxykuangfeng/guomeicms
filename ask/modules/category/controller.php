...
class P8_Ask_Category_Controller extends P8_Controller{
	...
	function valid_data(&$POST){
		...
		$data['category_logo'] = isset($POST['category_logo']) ? attachment_url($POST['category_logo'], true) : '';
		$data['category_banner'] = isset($POST['category_banner']) ? attachment_url($POST['category_banner'], true) : '';
		$data['comstom_a'] = isset($POST['comstom_a']) ? html_entities($POST['comstom_a']) : '';
		$data['comstom_b'] = isset($POST['comstom_b']) ? html_entities($POST['comstom_b']) : '';
		$data['comstom_c'] = isset($POST['comstom_c']) ? html_entities($POST['comstom_c']) : '';
		$data['comstom_d'] = isset($POST['comstom_d']) ? html_entities($POST['comstom_d']) : '';
		return $data;
