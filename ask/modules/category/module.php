	...
	function cache($cache_all = true, $write_cache = true, $id = 0){
		...
		while($arr = $this->DB_master->fetch_array($query)){
			...
			if($write_cache){
				...
				$arr['comstom_a'] = isset($arr['comstom_a']) && $arr['comstom_a'] ? html_decode_entities(html_entity_decode($arr['comstom_a'],ENT_QUOTES)) : '';
				$arr['comstom_b'] = isset($arr['comstom_b']) && $arr['comstom_b'] ? html_decode_entities(html_entity_decode($arr['comstom_b'],ENT_QUOTES)) : '';
				$arr['comstom_c'] = isset($arr['comstom_c']) && $arr['comstom_c'] ? html_decode_entities(html_entity_decode($arr['comstom_c'],ENT_QUOTES)) : '';
				$arr['comstom_d'] = isset($arr['comstom_d']) && $arr['comstom_d'] ? html_decode_entities(html_entity_decode($arr['comstom_d'],ENT_QUOTES)) : '';
				$arr['frame'] = isset($arr['frame']) && $arr['frame'] ? attachment_url(html_decode_entities(html_entity_decode($arr['frame'],ENT_QUOTES)),false,true) : '';
				$arr['logo'] = isset($arr['category_logo']) && $arr['category_logo'] ? attachment_url(html_decode_entities(html_entity_decode($arr['category_logo'],ENT_QUOTES)),false,true) : '';
				$arr['banner'] = isset($arr['category_banner']) && $arr['category_banner'] ? attachment_url(html_decode_entities(html_entity_decode($arr['category_banner'],ENT_QUOTES)),false,true) : '';				
				$_categories[$arr['id']] = $this->categories[$arr['id']] = array(
					...
					'url' => $arr['url'],
					'frame' => $arr['frame'],
					'logo' => $arr['category_logo'],	
					'banner' => $arr['category_banner'],	
				);
				$datas[$arr['id']] = $arr;
			}else{
