...
if(isset($option['verify']) && $option['verify']!=2) $select->in('i.verify', $option['verify']);
if(isset($option['recommend']) && $option['recommend']!=2) $select->in('i.recommend', $option['recommend']);
if(isset($option['status']) &&  $option['status']!=2){
	if($option['status']==3){
...
if(isset($option['urgent']) && $option['urgent']!=2) $select->in('i.urgent', $option['urgent']);
if(isset($option['offercredit']) && $option['offercredit']!=2) $select->in('i.offercredit', $option['offercredit']);
//非指定ID
if(empty($option['ids'])){
	if($option['category']){
