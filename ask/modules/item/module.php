...
function __construct(&$system, $name){
	...
	$this->table_module_member = $this->system->TABLE_ . 'member';
...
function post(&$data){
	...
	//没问答的会员则先增加
	if($insert_id && $data['uid']){
		$duid = $data['uid'];
		if(!$this->DB_master->fetch_one("SELECT id FROM " . $this->table_module_member . " WHERE id='$duid'")){
			$member = $this->DB_master->fetch_one("SELECT id,username,role_id FROM " . $this->table_member . " WHERE id='$duid'");
			$this->DB_master->insert(
					$this->table_module_member,
					array('id' => $duid,'verify' => 1,'username'=> $member['username'],'role_id'=>$member['role_id']),
					array('multiple' => array(), 'replace' => false)
			);
		}
	}
	if($insert_id){
...
function post_poller(&$data){
	...
	if($insert_id && $data['uid']){
		...
		if(!$this->DB_master->fetch_one("SELECT id FROM " . $this->table_module_member . " WHERE id='$duid'")){
			...
			$this->DB_master->insert(
					$this->table_module_member,
					array('id' => $duid,'verify' => 0,'username'=> $member['username'],'role_id'=>$member['role_id']),
					array('multiple' => array(), 'replace' => false)
			);
		}
	}
...
function cache_statistics(){
	$item_count = $this->DB_slave->fetch_one("SELECT COUNT(id) AS CC FROM $this->table");
	$salve_count = $this->DB_slave->fetch_one("SELECT COUNT(id) AS CC FROM $this->table WHERE status=3");
	$status= $this->DB_master->update(
			$this->system->table_statistics,
			array('item_count'=>$item_count['CC'],'solve_item_count'=>$salve_count['CC']),
			'',
			false
		);
	if(!$status){
		$this->DB_master->insert($this->system->table_statistics,array('item_count'=>$item_count['CC'],'solve_item_count'=>$salve_count['CC']));
	}
	$this->system->cache_statistics();	
}
