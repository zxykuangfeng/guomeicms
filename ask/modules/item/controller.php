...
class P8_Ask_item_Controller extends P8_Controller{
	 ...
	 * @param array &$POST 数据
	 * @param array $POST['ids'] 问题ID数组
	 * @param int $POST['status'] 1:未解决,2:续问,3:已解决
	 */
	function status(&$POST){
		$ids_arr = array();
		$allids = $comma = '';

		if(empty($POST) || !$this->valid_array($POST) || empty($POST['ids']) || !$this->valid_array($POST['ids']) || !isset($POST['status']) || !in_array($POST['status'],array(0,1,2,3))){
			return array();
		}
		...
		$this->model->solved("id IN($allids)", $POST['status']);
		return $ids_arr;
