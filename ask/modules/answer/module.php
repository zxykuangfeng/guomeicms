...
function set_bestanswer_summary($condition){
	...
	if($row){
		if($item->check_exists("id='".$row['tid']."'")){
			$ids_arr[] = $row['id'];
			$row['content'] = p8_cutstr(strip_tags(str_replace(' ','',html_decode_entities(html_entity_decode($row['content'],ENT_QUOTES)))),60);
...
function update(&$data){
	...
	$array['summary'] = P8_cutstr(strip_tags(html_decode_entities(html_entity_decode($row['content'],ENT_QUOTES))),30);
...
function show($tid = '', $cid = ''){
	...
	while($row = $this->DB_master->fetch_array($query)){
		...
		if($row['bestanswer']){
			...
			$data['best'][] = $row;
		}elseif(!$row['anonymous'] && ($this->core->controller($this)->check_category_action('verify', $cid) ||
				$member->check_exists_expertor('id='.$row['uid']))){
			$data['super'][] = $row;
