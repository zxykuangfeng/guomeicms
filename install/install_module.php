...
installog($this_system->name, $this_module->name,'delete old config');
...
installog($this_system->name, $this_module->name,'make dir');
//在系统安装模块
rm(CACHE_PATH . $this_system->name .'/modules/'. $this_module->name);
...
installog($this_system->name, $this_module->name,'update status');
$DB_master->update(
	$core->TABLE_ .'module',
	array(
		'installed' => 1,
...
installog($this_system->name, $this_module->name,'exe sql');
//数据表
$sql = get_install_sql(
	$DB_master,
...
installog($this_system->name, $this_module->name,'exe pre_install');
//自定义预安装操作
...
installog($this_system->name, $this_module->name,'make admin menuu');
...
if($admin_menus = @include $this_module->path .'install/admin_menus.php'){
	...
	insert_admin_menu($admin_menus, $admin_menus['menuid']?$admin_menus['menuid']:$menu['id']);
	//$admin_menu->cache();
	//添加菜单
}
...
installog($this_system->name, $this_module->name,'meke member menus');
...
installog($this_system->name, $this_module->name,'make homepage menu');
...
installog($this_system->name, $this_module->name,'make acls');
//权限
...
installog($this_system->name, $this_module->name,'make homepage_block');
//主页显示块
...
installog($this_system->name, $this_module->name,'make post_install');
//自定义完成安装操作
...
installog($this_system->name, $this_module->name,'make label_sql');
...
installog($this_system->name, $this_module->name,'financh');