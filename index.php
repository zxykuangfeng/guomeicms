...
if(empty($core->CONFIG['site_open'])){
	message($core->CONFIG['site_close_reason']);
    return;
}

if(defined('FROM_MOBILE')){
    if(empty($core->CONFIG['enable_mobile']) && !$IS_ADMIN){
        message('mobile is no enable', $core->url);exit;
    }
    $core->ismobile = true;
}
if($_GET['ismobile']==0) $core->ismobile = false;
...
//前端IP限制,信箱和表单不限
if($SYSTEM == 'core') {  
    if (!in_array($MODULE, array('forms', 'letter'))) {  
        front_ip();  
    }  
}else{  
    front_ip();  
