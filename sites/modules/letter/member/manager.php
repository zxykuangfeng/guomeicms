...
if($act=='search'){
	...
	$cates['type'] = $types = $this_module->get_category('type');
...
//print_r($my_manage);
	...
	if($status!='-1' && $status!=NULL)$select->in('status',trim($status));
...
elseif($act=='delete'){
	...
	foreach($ids as $id){
		...
		if($this_controller->check_manage($data['department'],$data['type']))
			$dids[]=$id;
	}
	if($dids)
...
elseif($act=='verify'){
	...
	foreach($ids as $id){
		$data = $this_module->getData($id);
		if($data['status']==0 && $this_controller->check_manage($data['department'],$data['type']))
			$dids[]=$id;
	}
	if($dids)
