...
class P8_Sites_Model_Controller extends P8_Controller{
...
function add(&$POST){
	...
	$data['site']= $this->model->system->SITE;
...
function check_field_name($model, $name){
	...
	//字段不能与两个数据表的重复
    $tmp = $this->DB_master->checkFileExists($item->table,$name);
	if(!empty($tmp)){
		return false;
	}

    $tmp = $this->DB_master->checkFileExists($item->addon_table,$name);
	if(!empty($tmp)){
		return false;
	}
