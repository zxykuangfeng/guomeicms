...
class P8_Sites_Model extends P8_Module{
...
function __construct(&$system, $name){
	...
	$this->field_types = array(
		'varchar'		=> 'sites_model_field_type_varchar',
		'tinyint'		=> 'sites_model_field_type_tinyint',
		'smallint'		=> 'sites_model_field_type_smallint',
		'mediumint'		=> 'sites_model_field_type_mediumint',
		'int'			=> 'sites_model_field_type_int',
		'bigint'		=> 'sites_model_field_type_bigint',
		
		'decimal'		=> 'sites_model_field_type_decimal',
		
		'char'			=> 'sites_model_field_type_char',
		
		
		'text'			=> 'sites_model_field_type_text',
		'mediumtext'	=> 'sites_model_field_type_mediumtext',
		'longtext' 		=> 'sites_model_field_type_longtext'
	);
	
	//可选的输入类型
	$this->widgets = array(
		'text'			=> 'sites_model_widget_text',
		'textarea'		=> 'sites_model_widget_textarea',
		'textdate'		=> 'sites_model_widget_textdate',
		
		'radio'			=> 'sites_model_widget_radio',
		'checkbox'		=> 'sites_model_widget_checkbox',
		
		'multi_select'	=> 'sites_model_widget_multi_select',
		'select'		=> 'sites_model_widget_select',
		
		'uploader'		=> 'sites_model_widget_uploader',
		'multi_uploader'=> 'sites_model_widget_multi_uploader',
		'image_uploader'=> 'sites_model_widget_image_uploader',
		'video_uploader'=> 'sites_model_widget_video_uploader',
		'video_multi_uploader'=> 'sites_model_widget_video_multi_uploader',
		'editor'		=> 'sites_model_widget_editor',
		'editor_basic'	=> 'sites_model_widget_editor_basic',
		'editor_common'	=> 'sites_model_widget_editor_common',
		'ueditor_basic'	=> 'sites_model_widget_ueditor_basic',
		'ueditor_common'=> 'sites_model_widget_ueditor_common',
		
		'city' 			=> 'sites_model_widget_city',
		'news_paper_map' => 'sites_model_widget_news_paper_map',
		'vote' 			=> 'sites_model_widget_vote',
		'vscode' 		=> 'sites_model_widget_vscode',
        
        'ckeditor'		=> 'sites_model_widget_ckeditor',
		'ckeditor_basic'	=> 'sites_model_widget_ckeditor_basic',
		'ckeditor_common'	=> 'sites_model_widget_ckeditor_common',
	);
}
...
function add(&$data, $addon_data = array()){
	...
	if(
		...
		/** 复制模板, 如果模板已经存在就不用覆盖 **/
		$farm = &$this->system->load_module('farm');
		$templates = $farm->get_sites_templates();
		foreach($templates as $template => $alias){
			$temp_dir = TEMPLATE_PATH . $this->system->name .'/'.$template .'/'.$this->system->name .'/'. $item->name .'/';
			...
			//创建风格文件夹
			md(PHP168_PATH .'skin/'. $this->system->name .'/'.$template .'/'. $this->system->name .'/'. $item->name .'/'. $data['name']);
		}
		//标签模板
...
function export($name, $new_name = ''){
    ...
    $farm = &$this->system->load_module('farm');
	$templates = $farm->get_sites_templates();
	foreach($templates as $template => $tdata){
		//前台模板
		$copies[TEMPLATE_PATH . '/'. $SYSTEM .'/'.$template .'/'. $SYSTEM .'/item/'. $name .'/'] = 
			$path .'template/'. '/'. $SYSTEM .'/'.$template .'/'. $SYSTEM .'/item/'. $new_name .'/';
		
		//风格
		$copies[PHP168_PATH .'skin/' . '/'. $SYSTEM .'/'.$template .'/item/'. $name .'/']
			= $path .'skin/' . '/'. $SYSTEM .'/'.$template .'/'.'/item/'. $new_name .'/';
	}
...
**/
function import($name, $alias, $copy=false){
...
function add_field(&$data){
	...
	if(
		...
		if($data['list_table']){
			$status = $this->DB_master->query($this->DB_master->formatTable("ALTER TABLE $item->table ADD `$data[name]` $field"));
		}else{
			$status = $this->DB_master->query($this->DB_master->formatTable("ALTER TABLE $item->addon_table ADD `$data[name]` $field"));
		}
...
**/
function cache($name = '',$site=''){
	parent::cache();
	$where = '';
	if($site)$where = " where site='$site'";
	$models = $this->DB_master->fetch_all("SELECT * FROM $this->table $where ORDER BY display_order DESC");
	...
	foreach($models as $model){
		...
		//读取各模型的字段
		$fields = $this->DB_master->fetch_all("SELECT id, name, parent, alias, type, part, not_null, length, is_unsigned, editable, default_value, widget, widget_addon_attr, data, list_table, filterable, orderby, config, units, description
