...
$sc = 'c';
$push_style = isset($_GET['push_style']) ? intval($_GET['push_style']) : 0;
$to_site_alias = isset($_GET['to_site_alias']) ? xss_clear(trim($_GET['to_site_alias'])) : '';
$site = $this_system->SITE;
$cid = isset($_GET['cid']) ? intval($_GET['cid']) : 0;
$model = isset($_GET['model']) ? xss_clear(trim($_GET['model'])) : '';
$keyword = isset($_GET['word']) ? xss_clear(trim($_GET['word'])) : '';
$username = isset($_GET['username']) ? xss_clear(trim($_GET['username'])) : '';
$push_username = isset($_GET['push_username']) ? xss_clear(trim($_GET['push_username'])) : '';
$mypu = isset($_GET['mypu']) && $_GET['mypu'] ? 1 : 0;
$push_username = $mypu ? $USERNAME : $push_username;
$verifier = isset($_GET['verifier']) ? xss_clear(trim($_GET['verifier'])) : '';
$class[88]='class="over"';
if($push_style) $page_url = $this_url .'?push_style='.$push_style;
$stc = 'c';
$allsites  = $this_system->get_sites();
$site_info = $allsites[$site];
$site_domain = $site_info['ipordomain'] ? $site_info['domain'].'/' : $core->CONFIG['url'].'/s.php/'.$site.'/';
$select = select();
$select->from($this_module->table.' as c', 'c.*');
if($to_site_alias){
...
if($push_style==1){
	//only to main	
	$select->in('c.to','cms');
}else if($push_style==2){
	//site to site
	$select->left_join($this_module->table.' as s', 's.site as push_sites','s.item_id=c.item_id');	
...
if($push_username){
	$select->in('c.push_username',$push_username);
}
if($keyword){
...
foreach($list as $key=>$item){
	...
	if($item['status'] != 1){		
		if($item['to'] == 'cms'){
			$sql = 'SELECT * FROM '. $core->TABLE_ .'cms_item_unverified' .' WHERE id = \''. $new_id .'\'';			
	...
	if($item['new_id']){
		if($item['to']=='cms'){
			...
			if($site_num==2) {
				$div_string = '';
				foreach($site_status as $site_tmp){
					$list[$key]['to_sites'] .= $div_string.!empty($allsites[$site_tmp]['sitename']) ? $allsites[$site_tmp]['sitename']  : '';
					$div_string = '<br>';
				}
			}
			if($site_num>=3) $list[$key]['to_sites'] = '多个分站';			
			*/
		}
	}
}
