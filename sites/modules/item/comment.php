...
if(empty($this_module->CONFIG['comment']['enabled'])){
	message($P8LANG['sites_item']['comment_denied']);
}
...
$iid or message($P8LANG['sites_item']['comment_denied']);
...
if(empty($data) || empty($data['allow_comment'])) message($P8LANG['sites_item']['comment_denied']);
...
case 'add':
	...
	if(empty($check) || empty($check['allow_comment'])) message($P8LANG['sites_item']['comment_denied']);
	...
	if(empty($UID)){
		...
		$ip = new IpLocation();
		$ipf = $ip->get(P8_IP);
		$USERNAME = $ipf['country'] . $P8LANG['sites_item']['comment_e-friend'];
	}
	...
	if($front)
		exit('<script type="text/javascript">alert("'. $P8LANG['sites_item']['comment_success'] .'"); window.parent.change_count(); </script>');
	else
		exit('<script type="text/javascript">alert("'. $P8LANG['sites_item']['comment_success'] .'");window.parent.change_count();window.parent.insert_comment('. p8_json($data) .');window.parent.location.hash = \'top\';</script>');
break;
