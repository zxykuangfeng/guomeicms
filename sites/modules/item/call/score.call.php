...
defined('PHP168_PATH') or die();
	...
	$query = $this->DB_master->query("SELECT `id`,`uid`,`username`,`title`,`site` FROM $T where id IN ($ids)");
	$message = $this->core->load_module('message');
	$credit = $this->core->load_module('credit');
	while($arr = $this->DB_master->fetch_array($query)){
		$log[] = array($arr['uid'], 3, intval($value), P8_TIME,$this->system->name,'item',$arr['site'],$arr['id'],$push_back_reason);
		//发站内信件
		if($arr['uid'] && $arr['uid'] != $UID) {
			$m = array(
				'username' => $arr['username'],
				'title' => $P8LANG['sites_item']['set_score'].'-'.$arr['title'],
				'content' => p8lang($P8LANG['sites_item']['changed_message'], $arr['title'], intval($value), $push_back_reason),
				'system' => true
			);
			$message->send($m);
		}
