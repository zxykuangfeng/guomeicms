...
//P8_CMS_Item::delete($data)
	...
	$query = $this->DB_master->query("SELECT $T.id, $T.site, $T.cid, $T.uid, $T.model, m.role_id, $T.pages, $T.timestamp FROM $T
		LEFT JOIN {$this->system->member_table} m ON $T.uid = m.id
		WHERE $data[where]");
	
	$ids = $comma = '';
	$credit_info = $id = $models = $hd = array();
	
	//加载分类模块
	$category = &$this->system->load_module('category');
	$category->get_cache();
	$category_item_count = $user_item_count = $delete_file = array();	
	$item_config = &$this->core->get_config($this->system->name, $this->name);
	...
	while($arr = $this->DB_master->fetch_array($query)){
		...
		if($arr['site'] != $this_site){
			$this->system->load_site($arr['site']);
			$category->get_cache();
			$change_site = true;
		}
		if(!empty($item_config['sync_del_html']) && !empty($category->categories[$arr['cid']]['htmlize'])){
			//获取要删除的HTML文件
		...
		if($change_site) $this->system->load_site($this_site);
	}
