...
if(!$verified){
    ...
    if(!empty($core->modules['auditflow']) && $core->modules['auditflow']['enabled'] && !empty(intval($core->CONFIG['audit_flow_enable_'.$this_system->SITE]))){
        ...
        $auditFlowEr = $auditFlow->checkVerfier($UID,$this_system->name, $this_system->SITE);
    }
	$IS_ADMIN || $auditFlowEr || $data['uid'] == $UID || in_array($this_system->SITE,$this_system->get_manage_sites()) || $this_controller->check_category_action('verify', $data['cid']) || $this_controller->check_category_action('verify_first', $data['cid']) or message('no_such_item_or_unverify');
}

$data or message('no_such_item_or_unverify');
//检查当前分类权限
if(!$this_controller->check_category_action($ACTION, $data['cid']) && !defined('P8_GENERATE_HTML')) {
...
if(!empty($SITE['config']['forbidden_dynamic']) && !($IS_ADMIN || defined('P8_GENERATE_HTML'))){
	...
	if($CAT['htmlize']){
		$data['#category'] = &$CAT;
		header('Location: '. $this_system->site_p8_url($this_module, $data, 'view'));
		exit;
...
if(!$IS_ADMIN && $SITE['config']['need_password']){
	...
	if(!$IS_ADMIN && $CAT['need_password']){
		//如果有密码，则检验密码
		//优先从cookie中获取密码，没有则使用用户输入的密码进行验证
		$cookie_password = get_cookie('SITE_PASSWORD_'.$data['cid']) ? get_cookie('SITE_PASSWORD_'.$data['cid']) : '';
		$password = isset($_POST['password']) ? trim($_POST['password']) : $cookie_password;
		...
		if($password && empty($cookie_password)) {
			$_config_ = &$core->CONFIG['cookie'];
			$_cookie_name = isset($_config_['prefix']) ? $_config_['prefix'].'SITE_PASSWORD_'.$data['cid'] : 'SITE_PASSWORD_'.$data['cid'];
			setcookie($_cookie_name,$password);
			set_cookie('SITE_PASSWORD_'.$data['cid'],$password);			
		}
	}
}
...
if($pay_check && !defined('P8_GENERATE_HTML')){
	...
	$page_url = $this_system->site_p8_url($this_module, $data, 'view', false);
...
if(
	...
	if(empty($next_item) ){
		...
		$next_item['subject'] = p8_cutstr($next_item['title'], 80);
		$next_item['#category'] = &$CAT;
		$next_item['url'] = empty($next_item['url']) ? $this_system->site_p8_url($this_module, $next_item, 'view') : $next_item['url'];
	}
	...
	if(empty($prev_item) ){
		...
		$prev_item['subject'] = p8_cutstr($prev_item['title'], 80);
		$prev_item['#category'] = &$CAT;
		$prev_item['url'] = empty($prev_item['url']) ? $this_system->site_p8_url($this_module, $prev_item, 'view') : $prev_item['url'];
	}
	$temp = array();
...
switch($title_style){
	case 1: $TITLE = $data['title'] .'_'. $SITE['config']['title']; break;
	case 2: $TITLE = $data['title'] .'_'. $CAT['name']; break;
	default: case 3: $TITLE = $data['title'] .'_'. $CAT['name'] .'_'. $SITE['config']['title']; break;
}
...
if(ISMOBILE)$CAT['view_template'] = $CAT['view_template_mobile'];
/*字段权限控制*/
