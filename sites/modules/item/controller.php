...
class P8_Site_Item_Controller extends P8_Controller{
...
function _init(){
	$this->category_acl = $this->get_acl('category_acl',$this->model->system->SITE);
}
function check_action($action, $prix='',$is_verify = false){
	global $IS_FOUNDER;
	if($IS_FOUNDER) return true;
	if(!$is_verify && $this->model->system->check_manager($action,$prix))return true;
	if(!$is_verify && $this->model->system->check_poster($action,$prix))return true;
	return parent::check_action($action, $prix?$prix:$this->model->system->SITE);
	
}
function check_admin_action($action, $prix = '',$is_verify = false){
	global $IS_FOUNDER;
	if($IS_FOUNDER) return true;
	if(!$is_verify && $this->model->system->check_manager($action,$prix))return true;
...
function check_category_action($action, $cid = 0,$site='',$is_verify = false){
	global $IS_FOUNDER;
	if($IS_FOUNDER) return true;
	if(!$is_verify && $this->model->system->check_manager($action,$site))return true;
    if($this->model->system->check_poster($action,$site))return true;
    if($this->allow_guest($action,$site))return true;
	if(!parent::check_action($action,$site?$site:$this->model->system->SITE)) return false;
    $category_acl = $site?$this->get_acl('category_acl',$site):$this->category_acl;

	if(!empty($category_acl[0]['actions'][$action])) return true;
	
	return !empty($category_acl[$cid]['actions'][$action]);
}

function allow_guest($action,$site){
     if(in_array($action,array('view','list','search')))return true;
}
/**
 * 允许IP
 */
...
function add(&$POST,$push = false,$filter_word_enable = false){
	...
	//检查权限,站群免检
	if(!defined('P8_CLUSTER') && !$push &&
	!$this->check_category_action('add', $POST['cid'])){
		return false;
	}
	...
	$data['main']['site'] = $data['item']['site'] = $data['addon']['site'] = $this->model->system->SITE;
	$data['push_item_id'] = $push ? $POST['push_item_id'] : 0;
	//不允许设定追加自增ID
	unset($data['addon']['id']);
...
function update($id, &$POST, $verified = true, $push = false){
	...
	if(!$push && $UID != $orig_data['uid']){
		if(!$this->check_category_action('update',$orig_data['cid'],$orig_data['site'])){
			return false;
		}
	}
	$have_frame = isset($POST['attribute'][6]) ? true : false;
	//验证数据	
	$data = $this->valid_data($POST);
	//编辑时允许去掉图片属性
	if(!$have_frame && $data['main']['attributes']){
		$arr = array_flip(explode(',',$data['main']['attributes']));
		unset($arr[6]);
		$arr = array_flip($arr);
		$data['main']['attributes'] = $data['item']['attributes'] = implode(',',$arr);
	}
	$data['addon']['addon_summary'] = &$data['main']['summary'];
	$data['addon']['addon_frame'] = &$data['main']['frame'];
...
function update_addon(&$POST){
	...
	$orig_data = $this->DB_master->fetch_one("SELECT a.*, i.uid, m.role_id,i.site, i.cid FROM {$this->model->addon_table} AS a
...
function verify($data){
	...
	WHERE $data[where]");
	/*
    $mysites = $this->model->system->get_manage_sites();
    if(!$mysites) return array();
	$mysites_s = $split = '';
	foreach($mysites as $site){
		$mysites_s .= $split."'".$site."'";
		$split = ',';
	}
	*/
	$ids = $comma = '';
	...
	if($ids){
		$this->DB_master->update(
			...
			"`to` = 'sites' and `new_id` IN ($ids)"
		);
	}
	
	//不是分站管理员也要来审核，呵呵
	//$data['where'] .= "  AND $T.site IN ($mysites_s)";
...
function delete($data){
	...
	$query = $this->DB_master->query("SELECT
	$T.id, $T.cid,$T.site, $T.uid FROM $T 
	WHERE $data[where]");
	
	$ids = $comma = '';
	while($arr = $this->DB_master->fetch_array($query)){
		//检查权限
		if(!$this->check_category_action('delete', $arr['cid'],$arr['site'])) continue;
...
function delete_addon($data){
	...
	if(!$this->check_category_action('delete', $item['cid'],$item['site'])) return false;
...
function valid_filter_word($content){
	...
	//正确词语提示
	if(count($matches)){
		$cache_right_word = $CACHE->read('', $core->name, 'word_filter_right');
		if($cache_right_word){
			foreach($matches as $keys=>$word){
...
function valid_censor_filter_word($content){
	...
	static $cache;
	if($cache === null){
		$cache = $CACHE->read('', $core->name, 'word_censor_filter');
	}
	if(!$cache || !$content){
		return array();
	}
...
function valid_data(&$POST,$push = false,$filter_enable = false){
	global $this_model, $IS_FOUNDER,$P8LANG,$core;
	...
	//推送数据到总站
	 $data['cluster_push_cid'] = !empty($POST['cluster_push_cid']) && $POST['cluster_push_cid']  ? intval($POST['cluster_push_cid']) : 0;
	//草稿箱
    ...
    $data['main']['frame'] = $data['main']['frame']?$data['main']['frame']:'';
	...
	$data['main']['source'] = isset($POST['source']) ? $func($POST['source']) : '';
	//评论
	...
	//发布时间,if push then do not to check,默认给权限设置
	//if($this->check_admin_action('create_time') || $push){
	...
	if(
		...
		$l1 || $data['main']['summary'] = $func($data['strip_tags_content']);
		
		$l2 || $data['addon']['addon_summary'] = $func($data['strip_tags_content']);
	}
