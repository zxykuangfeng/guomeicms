...
($this_controller->check_action($ACTION,$this_system->SITE) || $this_controller->check_action('sites_push',$this_system->SITE)) or message('no_privilege');
...
if(REQUEST_METHOD == 'POST'){
	...
	if(current($push_site)){
		$verify = $this_controller->check_category_action('autoverify', $cid, current($push_site));
	...
	//推送给分站数据需要审核
	if($this_module->CONFIG['sites_autoverify']){
		foreach($data as $key=>$item){
			$data[$key]['verify'] = 0;
			$data[$key]['verifier'] = '';
		}
	...
	//上推数据
	$count = $stop->push($data, $filter_word_enable,'sites','sites');
	if($count>0){
		//要设置属性的内容
