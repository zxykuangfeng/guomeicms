...
($this_controller->check_action($ACTION,$this_system->SITE) || $this_controller->check_action('sites_push_sites',$this_system->SITE)) or message('no_privilege');
...
if(REQUEST_METHOD == 'GET'){
	...
	*/
	...
	$sub_sites = $site_info['parent'] ? $farm->get_sub_site($site_info['parent']) : array();
	foreach($sub_sites as $site){
		$push_site[] = $site['alias'];
	}
	$parent_site_info = $site_info['parent'] ? $farm->get_parent_site($site_info['parent']) : '';
	
	if(!empty($parent_site_info)) $push_site[] = $parent_site_info['alias'];
	
	$push_site = array_unique($push_site);
	$allsites  = &$this_system->get_sites();
	/*
	foreach($allsites as $alias=>$site){
		if(!in_array($alias,$push_site)) unset($allsites[$alias]);
	}
	*/
	...
	//上推数据
	$count = $stop->push($data, $filter_word_enable);
	if($count>0){
		//要设置属性的内容
