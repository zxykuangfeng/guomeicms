...
if(empty($hash)){
	...
	$task = array(
		...
		'per_time' => $per_time,
		'offset' => 0,
		'verify' => empty($_POST['verify']) ? 0 : ($this_controller->check_admin_action('verify','',$independent_verify) ? 1 : 0),
		'delete' => $delete,
	);
...
if(empty($items)){
	...
	message($P8LANG['sites_item_spider_done'], $this_url);
}
...
function get_data(&$POST){
	...
	foreach($this_model['fields'] as $field => $v){
		...
		case 'multi_uploader': case 'uploader': case 'image_uploader': case 'video_multi_uploader':
...
_poster($hash, $P8LANG['sites_item_spidering']);
