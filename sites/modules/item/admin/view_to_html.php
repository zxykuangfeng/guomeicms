...
$this_system->check_manager('html') or message('no_privilege');
...
if(empty($_POST['task'])){
	...
	$cond = " WHERE site='{$this_system->SITE}' ";
	...
	//没有要生成的文件
	if(!$count){
		message('no_sites_item_to_html');exit;
	}
	
	$perpage = isset($_POST['perpage']) ? intval($_POST['perpage']) : 24;
	...
	for($i = 0; $i < $threads; $i++){
		...
		$iframe .= p8lang($P8LANG['sites_item']['htmlize']['thread'], array($i)) .
...
if($__page__ >= $__thread__['pages']){
	...
	echo p8lang($P8LANG['sites_item']['htmlize']['done'], array(P8_TIME - $__task__['time']));
	...
	_poster(
		$__task_name__,
		$_POST['thread_id'],
		p8lang($P8LANG['sites_item']['htmlize']['view_process'], $__thread__['page'], $__thread__['pages'])
	);
}
//##线程结束}
