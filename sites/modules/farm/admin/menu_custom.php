...
if(REQUEST_METHOD == 'GET'){
	$menus = $this_module->get_custom_menu($this_system->SITE,false);
	include template($this_module, 'menu_custom', 'admin');
	...
	switch($type){
		case 'delete':
			...
			$json = $this_module->delete_custom_menu($id);	
			echo jsonencode($json);
			exit;
		break;
		case 'cache':
			$this_module->menu_custom_cache($this_system->SITE);
			exit('done');
		break;
		default:
			$_POST = p8_stripslashes2($_POST);
			foreach($_POST['option'] as $key=>$item){	
				...
				foreach($item['id'] as $k=>$id){
					...
					if($id){
						//更新时						
						$DB_master->update(
							$this_module->table_menu_custom,
							$data,
							"id = '$id'"
						);				
					}else{
						//新增				
						$this_module->add_custom_menu($data);
					}			
				}
			}
			//显示
