...
if(REQUEST_METHOD == 'GET'){
	$id = intval($_GET['id']);
	$menus = $this_module->get_menu($this_system->SITE);
	
	$data = $this_module->DB_master->fetch_one("SELECT * FROM {$this_module->table_menu} WHERE id='$id' AND site='{$this_system->SITE}'");
	$data['summary'] = !empty($data['summary']) ? str_replace("<br>","\r\n",$data['summary']) : '';
	$data['frame'] = !empty($data['frame']) ? attachment_url($data['frame']) : '';	
	...
	$this_system->log(array(
		'title' => $P8LANG['_module_update_menu_admin_log'],
		'request' => $_POST,
	));
	$this_module->update_menu(
		array(
			'name' => $name,
			'parent' => $parent,
			'color' => $color,
