...
if(REQUEST_METHOD == 'POST'){
	$site = $_POST['site'] ? trim($_POST['site']) : '';
	if($site){
		in_array(clear_special_char($site),array_keys($this_system->sites)) or message('Hack!');
		$change_sites_flag = false;
		$old_site = $this_system->SITE;
		if($this_system->SITE != $site){
	...
	*/
	if($site) $select->in('i.site', $this_system->SITE);
	...
	foreach($list as $key=>$value){
		...
		$fv['site'] = $value['site'];	
		$fv['sitename'] = $allsites[$value['site']]['sitename'] ? $allsites[$value['site']]['sitename'] : $value['site'];
		$fv['cid'] = $value['cid'];		
		$fv['category_name'] = $value['category_name'];
		$fv['title'] = $value['title'];
		$fv['turl'] = $value['url'] ? html_entity_decode($value['url'],ENT_QUOTES) : '';
		$fv['url'] =  str_replace("index.php","s.php",$core->controller).'/'.$value['site'].'/item-view-id-'.$value['id'].'?verified='.$value['verified'];			
		if(substr($value['url'],0,7)=='http://') $fv['url'] = html_entity_decode($value['url'],ENT_QUOTES);
		$fv['surl'] = $STATIC_URL.'/sites/html/'.$value['site'].'/'.date('Y_m',$value['timestamp']).'/'.date('d_H',$value['timestamp']).'/content-'.$fv['id'].'.html';		
		$fv['source'] = $value['source'];
		$fv['author'] = $value['author'].($value['author_x'] ? ','.$value['author_x']:'').($value['author_y'] ? ','.$value['author_y']:'').($value['author_z'] ? ','.$value['author_z']:'');
	...
	$csv->save();
	if($change_sites_flag) $this_system->load_site($old_site);
	exit;
