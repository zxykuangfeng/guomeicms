...
$system_main_table = $db_config['table_prefix'].'sites_item';
$statistic_views = $db_config['table_prefix'].'cms_statistic_sites_views';
$data = $DB_master->fetch_one("SELECT site, cid, views, comments, model FROM $system_main_table WHERE id = '$id'");
$data or exit('');
$model_main_table = $db_config['table_prefix'].'sites_item_'.$data['model'].'_';
if(
	$DB_master->update(
		$system_main_table,
		array('views' => 'views +1'),
	...
	$ret = $DB_master->update(
		$statistic_views,
		array('count' => 'count +1'),
		"cid = '$cid' and site = '$site' and timestamp = '$today'",
		false
	);
	if(!$ret){
		$DB_master->insert(
			$statistic_views,
			array('site' => $site, 'cid'=> $cid, 'timestamp'=> $today, 'count' => 1)
		);
	}
}
