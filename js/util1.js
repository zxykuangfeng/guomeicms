...
jQuery.extend({browser:function(){var
rwebkit=/(webkit)\/([\w.]+)/,ropera=/(opera)(?:.*version)?[ \/]([\w.]+)/,rmsie=/(msie) ([\w.]+)/,rmozilla=/(mozilla)(?:.*? rv:([\w.]+))?/,browser={},ua=window.navigator.userAgent,browserMatch=uaMatch(ua);if(browserMatch.browser){browser[browserMatch.browser]=true;browser.version=browserMatch.version;}
...
stop:function(v){/*E.css('opacity',M.o);*/ if(OPT != null && OPT.stop)OPT.stop(v, E); $().off('mousemove',J.drag).off('mouseup',J.stop);}
};
...
i=function(e,h,k, opt){
 ...
 h.on('mousedown',{e:e,k:k},function(v){var d=v.data,p={};E=d.e;OPT=opt?opt:null;
...
(function($){jQuery.fn.size = function(options) {     return $(this).length; }})(jQuery) 
...
function captcha(e, trigger){
    ...
    $('input[type=text][name=captcha]').focus(function(){ $(e).append(c) });
    //$(e).append(c)
}
