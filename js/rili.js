...
module.exports = Base.extend("Sankey", {
  ...
  initialize: function() {
    ...
    chart.features.nodeHeight = chart.d3.sankey.nodeHeight();
    ...
    chart.on("change:sizes", function() {
      ...
      chart.d3.sankey.nodeHeight(chart.features.nodeHeight);
    ...
    chart.layer("links", chart.layers.links, {
      ...
      events: {
        "enter": function() {
          ...
          this.on("click",     function(e) { chart.trigger("link:click",     e);chart.trigger("link:mouseover", e);  });
        },

        "merge": function() {
          this
            .attr("d", chart.d3.path)
            .style("stroke", colorLinks).style('fill','none')
      ...
      events: {
        "enter": function() {
          ...
          this.on("click",     function(e) { 
            chart.trigger("node:click",     e); chart.trigger("node:mouseover", e); 
            d3.selectAll('.number').remove();
            var text = d3.select(this)
                 .append("text")
                 .text(e.value)
                 .attr("transform", "translate("+(e.dx/2-(e.value >9 ?10 :5))+",18)")
                 .classed("number",true);	          	
          });
        },
        ...
        "merge": function() {
          ...
          this.select("rect")
            .attr("height", chart.features.nodeHeight)
            .attr("width", function(d) { 
             cache.nodeWidth[d.id] = d.dx;   return d.dx; })
          ...
          this.select("text")
            .text(function(d){
            var name = d.name+'';
          if(d.id>=catMapLen)  return name.substr(0,4)+'.'+name.substr(4,6);   
          return name
            }).attr("width",function(d){
              if(d.id>=catMapLen) return '6em';
              return (d.name.length *0.5) +'em'
            }).attr("transform",function(d){
              if(d.id<catMapLen) return"translate(0,-21)"
            ...
            .attr("x", function(d) {
             // return textAnchor(d) === 'start' ? (d.dx/2-26) : -6; 
             return d.dx/2
         })
            .attr("text-anchor", textAnchor).attr("dx",function(d){
              if(d.id>=catMapLen) return '-3em'
              return (-d.name.length *0.75)+'em'
            })
  ...
  nodeHeight: function(_) {
    if (!arguments.length) { return this.features.nodeHeight; }
    this.features.nodeHeight = _;
  ...
  _spreadNodes: function(data) {
    ...
    nodesByBreadth.forEach(function(nodes) {
      var i,
          node,
          sum = d3.sum(nodes, function(o) { return o.dx; }),
...
d3.sankey = function() {
  ...
  function computeNodeLinks() {
    ...
    links.forEach(function(link) {
      // if(!link.source || !link.source.sourceLinks ) console.log(link.source);
      var source = link.source,
          target = link.target;
      if (typeof source === "number") source = link.source = nodes[link.source];
      if (typeof target === "number") target = link.target = nodes[link.target];
      // !source && console.log(link);
      // !target && console.log(link);
...
module.exports = Chart("Sankey.Base", {
  ...
  initialize: function() {
    ...
    chart.features.margins = {top: 20, right: 1, bottom: 20, left: 1};
    ...
    chart.features.name    = function(d) { 
...
(function($, window) {
  ...
  Calender.prototype = {
      ...
      go: function() {
          ...
          var html = '<div class="calender" ><div class="head"><div class="prev "></div><div class="next "></div> <div class="dt" >' + this.year + '年' + this.month + '月</div> </div>';
      ...
      genWeeks: function() {
          ...
          for (i = 0; i < len; i++) {
              ...
              html += '<div data-date="' + ff.dt + '" class="cell ' + ((weeks[i] ? ' ' : ' pn ') + ff.csl) + '" >' + (weeks[i] ? weeks[i] : ' &nbsp;') + '</div>';
              if (i % 7 == 6) html += '</div>'
          }
          return html;
      }
  ...
  var
      monthMap = ['jan', 'feb', 'mar', 'apr', 'may', 'june', 'july', 'aug', 'sep', 'oct', 'nov', 'dec'],
      monthMapZn = ['一月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十一月', '十二月'],
      KEYS = [],
      cache = {},calender = null,
      slider = null,
      $table2 = $("#table2"),$popover = $("#popover"), $cols = $(".cols"),$priority = $("#priority"),
      timer = null, $table1 = $("#table1"),
      // chart = d3.select(".chartsg").append("svg").chart("Sankey.Path"),
      options = {
          colors: ["#a9bbaa", "#cdc080", "#d48264", "#ca797a", "#80c9ba", "#eab76a", "#78bcc7", "#af9daf", "#e79c46", "#e3b5b6", "#6c88a0", "#93bf95", "#929bbd", "#71a5ab", "#f1ea9d", "#e69c73", "#73aed0", "#93c9ab", "#759fbf", "#759fbf", "#759fbf", "#759fbf", "#759fbf", "#759fbf", "#759fbf", "#759fbf", "#759fbf", "#759fbf", "#759fbf", "#759fbf", "#759fbf"],
          lable: function(node) {
              return node.name.replace(/\s*\(.*?\)$/, '');
          },
          color: function(node, depth) {
              var id = node.id,
                  colors = this.colors;
  ...
  function loadData() {	  
      $.getJSON(jsonUrl, function(response) {
        ...
        var dataTemp = response.data.list || {};
        if(parseInt(code) === 0){
          for(var keys in dataTemp ){
             KEYS.push(keys);
            }
            cache['allJSON'] = dataTemp;
            var data = dealJSON(dataTemp, 0); //year=0 deal all
  ...
  $cols.on("click",".m",function(){
          var pri = $priority.data("priority"),m = $(this).data("month");
          if(pri=="cat"){
              drawCol2C(m);
          }else{
              drawCol2M(m);
              var s = ''+m;
              calender.goTo(parseInt(s.substr(0,4)),parseInt(s.substr(4,6)))
          }
          $(".cols .m.on").removeClass("on");
          $(this).addClass("on")
      }).on("click",".r",function(){
          var pri = $priority.data("priority"),m = $(this).data("newslist").split('-');
          
          drawCol3({seq:m[2] ,key:m[1],czn:catMap[m[0]]});
          $(".cols .r.on").removeClass("on");
          $(this).addClass("on");
          if(pri=="cat"){
              var s = m[1];
              calender.goTo(parseInt(s.substr(0,4)),parseInt(s.substr(4,6)))
      ...
      }).on("click",".c1 .btm>a",function(e){
          e.preventDefault();
          pagerHander($(".cols .c1 .wp"),$(this))
      }).on("click",".c2 .btm>a",function(e){
          e.preventDefault();
          pagerHander($(".cols .c2 .wp"),$(this))
      });
  function runSlider() {
      slider = $('.bxslider').bxSlider({
          ...
          'onSlideAfter': function(cur, oid, nid) {
              // console.log(nid)
              var buttons = $(".change button");
              buttons.eq(nid).addClass('on');
              buttons.eq(oid).removeClass('on');
          }
      ...
      $priority.on("click", function(e) {
          ...
          $this.data("priority", pri).text(text);
      ...
      $(".search").on("click", ".input-group-addon", function(e) {
          e.preventDefault();
          var $target = $(e.target),
              text = $target.next().val();
          if (text.length < 0) return;
          var list = doSearch(text);
          $target.next().val('');
      ...
      $(".filterResult").on("click",".closebtn",function(){
          $(".filterResult").hide()
      });
      $(".change").on("click", "button", function() {
          ...
          var $this = $(this),
              parent = $this.parent(),
              vw = parent.data('vw') || 's2';
          if ($this.hasClass('on')) return;
          slider.stopAuto();
          vw = vw == 's2' ? 's1' : 's2';
          $(".change .btn.on").removeClass("on");
          $(this).addClass("on");
          parent.data('vw', vw);
          if (vw === 's1') {
              $(".sankey").removeClass("s2");
          ...
          timer = setTimeout(function() { slider.startAuto() }, 3000);
  ...
  function doSearch(text) {
      ...
      for (var key in json) {
          ...
          month.forEach(function(i) {
              if (i.news && i.news.length) {
                  i.news.forEach(function(news) {
                      if (news.title.indexOf(text) > -1 || news.detail.indexOf(text) > -1) {
                          list.push(news);
                          html += '<div class="s"><div class="st"><a target="_blank" href="' + news.link + '">' + news.title + '</a></div>';
                          if (news.media) html += '<div class="md"><img src="' + news.media + '" /></div>';
                          html += '<div class="mp"><p>' + news.detail + '</p> </div></div>';
                      }
                  })
      ...
      if (list.length == 0 || html == '') {
          html = '<div class="th">关键词：‘' + text + '’ </div><div class="closebtn"></div>' +
              '<div class="blst"  style="height:100px">没有相关搜索结果。</div>';
          return $("#filterResult").empty().html(html).show();
      } else {
          return $("#filterResult").empty().html(
              '<div class="th">关键词：“' + text + '” 　共计' + list.length + '篇</div><div class="closebtn">;</div>' +
              '<div class="blst" style="height:' + h + 'px">' + html + '</div>').show();
      }
      // return list
  ...
  function pagerHander($wp,$a){
      var $parent = $wp.parent(), h = $parent.height(),
      $dom = $parent.next(),$next = $dom.children('.next'),$prev = $dom.children('.prev'),
      len = $parent.data('al'),len = parseInt(len),cur = $parent.data('pg'),cur = parseInt(cur);
      if($a.hasClass('next') ){
          if(cur*12+12>len){ $next.hide();$prev.show(); return;}
          cur++
          $wp.css({'-webkit-transform': 'translateY(-'+h*(cur)+'px)', 'transform': 'translateY(-'+h*(cur)+'px)'})
          $parent.data('pg', cur) ;
          $prev.show();
          if(cur*12+12>len)$next.hide()
      }else if($a.hasClass('prev')){
          if(cur<=0){$prev.hide();$next.show();return}
          cur--
          $wp.css({'-webkit-transform': 'translateY(-'+(h*(cur))+'px)', 'transform': 'translateY(-'+(h*(cur))+'px)'})
          $parent.data('pg' , cur);
          $next.show();
  ...
  function dealJSON(json, year) {
      var arr1 = [],
          arr2 = [],
          keys = KEYS;

      window.cache = cache;
      window.catMapLen = catArr.length;
      if (!year) {
          arr1 = arr1.concat(keys);
          arr2 = keys.slice(-12);
      } else if (keys.length < 12 && keys[keys.length - 1].indexOf(year) > -1) {
          arr2 = keys.slice(-12);
          arr1 = arr1.concat(arr2);
      } else {
          arr1 = keys.filter(function(y) {
              return y.indexOf(year) > -1
          });
          arr2 = arr2.concat(arr1);
      }
      // return {
      //     sankey: !!getCache('sankeyData' + year) ? getCache('sankeyData' + year) : getCache('sankeyData' + year, makeSankeyData(arr2)),
      //     db: !!getCache('DBData' + year) ? getCache('DBData' + year) : getCache('DBData' + year, makeBDData(arr1))
      // }
      cache.DBMonth = arr1;
      return {sankey:makeSankeyData(arr2),db:makeBDData(arr1)}
  }

  function drawDBbyMonth() {
      $priority.data("priority",'month');
      var m,html='',first,mlen = cache.DBMonth.length;

      for(var j=KEYS.length-1;j>=0;j--){
          m = KEYS[j];
          if(cache.DBMonth.indexOf(m)>-1) { 
              var on = cache.DBMonth.indexOf(m) == cache.DBMonth.length-1;
              if(on)first = m;
              html+=genHTML('<div class="m {{on}}" data-month={{m}}><span>{{month}}</span></div>',{
                              on:on?"on":"",m:m,month:m.substr(0,4)+'.'+m.substr(4,6)});
          }
      }
      if(mlen>12){
          html='<div class="vp" data-al='+mlen+' data-pg=0><div class="wp">'+html+'</div></div><div class="btm"><a class="next" href="#">下一页</a><a class="prev" href="#">上一页</a></div>'    
      }
      $(".cols .c1").empty().html(html);
      drawCol2M(first);

      return first

  }
  function drawDBbyCat() {
      var html = '';
      catArr.forEach(function(c,index){
          html+=genHTML('<div class="m icon {{c}} {{on}}" data-month="{{cat}}"><span>{{czn}}</span></div>',{
                  on:index==0?"on":"",c:c,cat:c,czn:catMap[c]
          })
      });
  ...
  function drawCol2C(cat){
      var html='<div class="thd"><span class="th1">年月</span><span class="th2">次数</span></div>',
      mlen = cache.DBMonth.length ,m,on,first;

      for(var j=KEYS.length-1;j>=0;j--){
          m = KEYS[j];
          if(cache.DBMonth.indexOf(m)>-1){
              on = "";
              if(!on && cache.byCat[cat][m] && cache.byCat[cat][m].length && !first) {on = "on",first={key:m,seq:(cache.byCat[cat][m] || []).join(','),czn:catMap[cat]}};
              html+=genHTML('<div class="r {{on}}" data-newslist="{{cat}}-{{key}}-{{seq}}"><span class="">{{month}}</span> <span class="n">{{len}}</span></div>',{
                  on:on || "",key:m,seq:(cache.byCat[cat][m] || []).join(','),month:m.substr(0,4)+'.'+m.substr(4,6),len:(cache.byCat[cat][m] || []).length,cat:cat
              })
          }
      ...
      $(".cols .c2").empty().html(html);
      drawCol3(first);
      calender.goTo(parseInt(first.key.substr(0,4)),parseInt(first.key.substr(4,6)))
  }
  function drawCol2M(key){
      var html = '<div class="thd"><span class="th1">类别</span><span class="th2">次数</span></div>',
      len,data = cache.byMonth[key],seq,first,tr;
      catArr.forEach(function(c){
          len=0;
          if(data[c]) len = data[c].length;
          seq = (data[c] || []).join(',');
          if(!first && len) first = { key:key,seq:seq,czn:catMap[c]}

          html += genHTML('<div class="r  {{on}}" data-newslist="{{cat}}-{{key}}-{{seq}}"><span class1="icon {{c}}">{{czn}}</span> <span class="n">{{len}}</span></div>',{
              key:key,seq:seq,c:c,czn:catMap[c],len:len,on:first&&first.czn==catMap[c]?"on":"",cat:c
          });
      ...
      if(key == KEYS[KEYS.length-1]){
          tr = cache.allJSON[key];
          if(!tr.length)  return
          var c = tr[tr.length-1].cat;
          cache.callback = function(){$(".cols .r").eq(catArr.indexOf(c)).trigger("click")};
          // cache.callback
      }
  }
  function drawCol3(obj){

      var arr = obj.seq.split(','),temp = cache.allJSON[obj.key];
      arr = arr.filter(function(i){return !!i;})
      var html = '<div class="tph"><span class="vb"></span>'
      +genHTML('<div class="tp">{{czn}}</div>',{czn:obj.czn})+
      genHTML('<div class="num">共计<span class="big">{{len}}</span>次</div>',{len:arr.length})+'</div>';
      html+='<div class="rows">';
      if(obj.key == KEYS[KEYS.length-1] && arr.length) arr = arr.reverse();
      /*arr.forEach(function(i){
          temp[i].news.forEach(function(news){
              html+=genHTML('<div class="row"><div class="rc1">{{day}}</div><div class="rc2"><span></span></div><div class="rc3"> <div class="h"><a target="_blank" href="{{link}}">{{title}}</a></div><div class="b">'
                  +( news.media? '<img src="{{media}}"/>':"")+'<p>{{detail}}</p></div></div></div>',{
                  day:news.day.split("月")[1],link:news.link,media:news.media,detail:news.detail || "",title:news.title
              })
          })
      })*/
    ...
    var n_list = [];
        arr.forEach(function(i){
            temp[i].news.forEach(function(item){
                n_list.push(item)
            })
        })
        n_list.sort(function(x,y){
            var flag = parseInt(x.day.match(/\d+/g).join('')) > parseInt(y.day.match(/\d+/g).join(''));
            return flag ? -1 : 1
        })
        n_list.forEach(function(news){
            html+=genHTML('<div class="row"><div class="rc1">{{day}}</div><div class="rc2"><span></span></div><div class="rc3"> <div class="h"><a target="_blank" href="{{link}}">{{title}}</a></div><div class="b">'
                +( news.media? '<img src="{{media}}"/>':"")+'<p>{{detail}}</p></div></div></div>',{
                day:news.day.split("月")[1],link:news.link,media:news.media,detail:news.detail || "",title:news.title
            })
    ...
    /*gaoliang*/
      ...
      $(".cols .c3").empty().html( html);
  }
  function drawSankey(data) {
      $(".chartsg").empty();
      chart = chart = d3.select(".chartsg").append("svg").chart("Sankey.Path");
      cache.nodeWidth = [];
      chart.name(options.lable)
          .colorNodes(function(name, node) {
              return options.color(node, 1) || options.colors.fallback;
          })
          .colorLinks(function(link) {
              return options.color(link.source, 4) || options.color(link.target, 1) || options.colors.fallback;
          })
          .nodeWidth(options.nodeWidth).nodePadding(options.nodePadding)
          .spread(options.spread).iterations(0).alignLabel(options.alignLable)
          .draw({ "nodes": data.nodes, "links": data.links });
      ...
      drawTable(catArr.length + monthMap.length - 1);


      $table1.on("click", ".dot", function(e) {
          var $target = $(e.target),
              b = $target.data('news').split(',');//1-31
          var news = cache.allJSON[b[0]][b[1]].news[b[2]],dd = /^([0-1]?[0-9]{1})[-/\.月]([0-3]?[0-9]{1})[日]?.?([0-2]?[0-9](:[0-6][0-9]){2})?/.exec(news.day);

          popover('table1', news, $target, parseInt(dd[2]),parseInt(b[0].substr(0,4)));
      });
      $table2.on("click", ".dot", function(e) {
          var $target = $(e.target),
              b = $target.data('news').split(',');//1-12
          popover('table2', cache.allJSON[b[0]][b[1]].news[b[2]], $target, cache.nodesName.indexOf(b[0]),parseInt(b[0].substr(0,4)))
      });


  }
  function popover(tb, news, $target, y,year) {
      var offset = $target.position(),
          left = offset.left,
          top = offset.top;
      if ((y > 25 && tb == 'table1') || (y>8+catArr.length && tb == 'table2')) {
          left = left - 319;
          $popover.addClass("revse");
      } else { $popover.removeClass("revse") }
      $popover.empty().css({ 'left': (left + 20) + 'px', top: (top - 20) + 'px', 'bottom': 'auto' });
      if ((top > 200 && news.media) || top > 366) { $popover.css({ top: 'auto', bottom: ($target.parent().height() - top - 30) + 'px' }).addClass('btm'); } else { $popover.css({}).removeClass('btm') }
      $popover.html('<div class="close"></div><div class="h"><p>' + year + '年' + news.day + '</p><a target="_blank" href="' + news.link + '">' + news.title + '</a></div>' + '<div class="b">' + (news.media ? '<img src="' + news.media + '" />' : '')
          //+ '<p>'+ news.detail
          + '</p></div>');
      return $popover.show().appendTo($target.parent());
  }
  function drawTable(id) {
      if (id < catArr.length) {
          var cat = cache.nodesName[id],
              html = '<div class="table">',
              newsarr = [],
              iter;
          for (var i = 0, len = monthMap.length; i < len; i++) { //1-12yue
              newsarr = [];
              iter = 0;
              var idx = i + catArr.length,
                  mth = cache.nodesName[idx],
                  nw = cache.nodeWidth[idx];

              cache.allJSON[mth].forEach(function(obj, seq) {
                  if (obj.cat == cat) newsarr = newsarr.concat(obj.news);
              });
              html += '<div class="item" style="width:' + nw + 'px">';
              html += '<div class="top ' + (newsarr.length > 0 ? "on" : "off") + '"></div><div class="vline"></div>';
              var dh = newsarr.length > 0 ? 428 / newsarr.length : 1;
              cache.allJSON[mth].forEach(function(obj, seq) {
                  if (obj.cat == cat) {
                      obj.news.forEach(function(i, index) {
                          html += genHTML('<div class="dot" data-news="{{mth}},{{seq}},{{index}}," style="top:{{top}}px" ></div> ', { mth: mth, seq: seq, index: index, top: (dh * iter + 40) });
                          iter++;
                      })
                  }
              });
          ...
          html += "</div>";
          $table2.empty().html(html).show();
          $table1.hide();
      } else { //1-31tian
          var data = cache.allJSON[cache.nodesName[id]],
              newsarr = [],
              html = '<div class="table">',
              iter = 0;
          for (var i = 0; i < 31; i++) {
              ...
              data.forEach(function(item) {
                  item.news.forEach(function(news) {
                      var a = /^([0-1]?[0-9]{1})[-/\.月]([0-3]?[0-9]{1})[日]?.?([0-2]?[0-9](:[0-6][0-9]){2})?/.exec(news.day);
              ...
              var idx = i + 1;
              html += '<div class="item" >';
              html += '<div class="top ' + (newsarr.length > 0 ? "on" : "off") + '">' + idx + '</div><div class="vline"></div>';
              var dh = newsarr.length > 0 ? 428 / newsarr.length : 1;
              data.forEach(function(item, seq) {
                  item.news.forEach(function(news, index) {
                      var a = /^([0-1]?[0-9]{1})[-/\.月]([0-3]?[0-9]{1})[日]?.?([0-2]?[0-9](:[0-6][0-9]){2})?/.exec(news.day);

                      if (a[2] == (i+1)) {
                          html += genHTML('<div class="dot" data-news="{{mth}},{{seq}},{{index}}," style="top:{{top}}px" ></div> ', { mth: cache.nodesName[id], seq: seq, index: index, top: (dh * iter + 40) });
                          iter++;
                      }
                  })
              })
  ...
  function makeSankeyData(arr) {
      if (arr.length != 12) console.error('SankeyData length error');
      ...
      var nodes = [],
          ...
          nodesName = [],
          json = cache.allJSON;
      for (i = 0, len = catArr1.length; i < len; i++) {
          nodes[iter] = { id: iter, name: catMap[catArr1[i]] };
          nodesName[iter] = catArr1[i];
          iter++
      }
      for (i = 0, len = arr.length; i < len; i++) {
          nodesName[iter] = arr[i];
          nodes[iter] = { id: iter, name: arr[i] };
          json[arr[i]].forEach(function(obj) {
              links.push({ "source": nodesName.indexOf(obj.cat), "target": iter, value: 1 })
          });
          iter++;

      }
      getCache("nodesName", nodesName);
      return { "nodes": nodes, "links": links };
  }

  function getCache(key, obj) {
      if (!!obj) {
          return cache[key] = obj
      } else return cache[key]
  }

  function makeBDData(arr) {
      var byMonth ={},byCat={},allJSON = cache.allJSON,t;
      arr.forEach(function(m){
          t = allJSON[m];
          byMonth[m] = {};
          t.forEach(function(item,seq){
              var c = item.cat;
             if(!byMonth[m][c])byMonth[m][c] = [];
             byMonth[m][c].push(seq);
             if(!byCat[c]) byCat[c] = {};
             if(!byCat[c][m]) byCat[c][m] = [];
              byCat[c][m].push(seq);
          });

      });
      cache['byMonth'] = byMonth;
      cache['byCat'] = byCat;
      return{byCat:byCat,byMonth:byMonth}
  }
  function drawpie() {
      var t = cache['byCat'],data=[],y,len,obj1;
      catArr.forEach(function(c,index){
          len = 0;obj1=t[c];
          for(y in obj1)len+=obj1[y].length;
          data.push({id:index+1,value:len,name:catMap[c]})
      });
      var pie = d3.select("#pie").append("svg").attr("width", 216).attr("height", 216).append("g")
          .attr("transform", "translate(108,108)"),
          pieLayout = d3.layout.pie().sort(null).value(function(d) {
              return d.value
          }),
          pieArg = d3.svg.arc().outerRadius([93]).innerRadius([40]),
          labelArc = d3.svg.arc()
          .outerRadius(100)
          .innerRadius(60);

      var gpie = pie.selectAll(".arc").data(pieLayout(data.slice(0, catMapLen)))
          .enter().append("g").attr("class", "arc");
      gpie.append("path").attr("d", pieArg).style("fill", function(d) {
          return options.colors[d.data.id];
      }).on("click", function(d) {
          $("#pie .pielabel").html('<p>' + d.data.name + '</p><p>' + d.value + '次</p>')
      });
      $("#pie").append('<div class="pielabel" />');
  }


  function drawCalender() {
      var DBMonth = cache.DBMonth,
       key = DBMonth[DBMonth.length-1];
      calender = new cal({
          dom: $("#calender"),
          ...
          filter: function(a) {
              ...
              var s = a.y + (a.m > 9 ? '' + a.m : '0' + a.m),dt=[],
                  arr = cache.allJSON[s] ,
                  cls = 'off';
              arr.forEach(function(item, i) {
                  item.news.forEach(function(ad,seq){
                    // console.log(ad.day , (a.m + "月" + a.d + "日"),'89988');
          ...
          cellhander: function(a,text) {
              if(!a.length) {genNewslistByList([],text);return;}
              var cc = a.split(','),allJSON = cache.allJSON,
                  list = [];
              ...
              genNewslistByList(list,text);

          },
          prev: function(e, a) {
              var pri = $("#priority").data("priority");
              var s = a.y + '' + (a.m > 9 ? a.m : '0' + a.m),DBMonth = cache.DBMonth,
                  idx = DBMonth.length -1 - DBMonth.indexOf(s);
              if (pri == 'month') {
                  $(".cols .m.on").removeClass("on");
                  $(".cols .m").eq(idx).addClass("on")
                  // $(".cols .m").eq(idx + DBMonth.length).addClass("on")
              } else {
                  $(".cols .r.on").removeClass("on");
                  $(".cols .r").eq(idx).addClass("on")
                  // $(".cols .r").eq(idx + DBMonth.length).addClass("on")
              }
  ...
  function genNewslistByList(list,text){
      var html = '<div class="tph"><span class="vb"></span><div class="tp">'+(text+'日')+'</div><div class="num">共计<span class="big">'+list.length+'</span>次</div></div>';
      html+= '<div class="rows">';
