...
function ShowTopMenu(id)
{
	...
	for(var i in json.menus){
		iids = json.menus[i].id;
		if(MENU_DENIED[iids]) continue;
...
function ShowSecMenu(id,obj){
	...
	for(var i in json.menus){
		...
		for(var j in _json.menus){
			_iids = _json.menus[j].id;		
			if(MENU_DENIED[_iids]) continue;
			...
			url = _json.menus[j].url;
			if(url.indexOf('u.php/sites/')>-1){
				if(url.indexOf('?')>-1)ls='&';
				if(site_tmp) url=url+ls+'site='+site_tmp;
			}
			menu += '<li><a href="'+url+'" target="'+target+'" class="lm'+_json.menus[j].id+'">' + _json.menus[j].name + '</a></li>';
		}
...
function member_labelshows(id){
		...
		$('#'+id).append('&nbsp;<a href="'+P8CONFIG.URI.core.member.U_controller+'/../cms/item-reflash_index" id="edit_label">['+ P8LANG.refresh_page +']</a>');
	}
...
function init_help(context){
	...
	$('.help', context).each(function(i){			 
		$(this).before(
			$('<img src="/skin/admin/help_icon.gif" class="helpicon" />').on('click',
                function(){
