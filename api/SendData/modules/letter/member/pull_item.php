...
$id= intval($_GET['id']);
if(!$id) message('error',$this_router .'-pull_data',3);
...
if(file_exists($xmlfile)){
	...
	curl_close($curl);
	if($ret) {
		$this_module->set_pull($id,2);
		message('推送成功',$this_router .'-pull_data',3);
	}else{
		message('error，推送失败',$this_router .'-pull_data',3);
	}
}else{
	message('error，XML文件没生成成功，请检查目录权限',$this_router .'-pull_data',3);
}
?>