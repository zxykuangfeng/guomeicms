...
$sites_list = $DB_master->fetch_all("select i.status,i.site,s.sitename,count(*) as count from ".$stop_data_table." as i left JOIN ".$site_table." as s on s.alias = i.site where i.from = 'sites' and i.to = 'cms' and i.status = 1 GROUP by site order by count desc limit 10");
