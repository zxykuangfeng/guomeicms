...
$sites_count = array();
$sites_list = $DB_master->fetch_all("select i.site,s.sitename,count(*) as count from ".$main_table." as i left JOIN ".$site_table." as s on s.alias = i.site where s.status = 1 GROUP by site order by count desc LIMIT 10");
//排序
if(empty($direct)){
	$sort = array_column($sites_list,'count');
	array_multisort($sort,SORT_ASC,$sites_list);
}
/*
* 默认读10个站点
