...
if(REQUEST_METHOD == 'GET'){
	...
	*/
	...
	$file = real_path(PHP168_PATH . $base_dir . $template .'.css');
	$file = str_replace(array("./","../",".\\","..\\","..","\\"),'',$file);
	strtolower(file_ext($file)) == 'css' or message('access_denied');
	$file or message('access_denied');
	stristr($file, PHP168_PATH) == $file or message('access_denied');
	...
	include template($core, 'edit_css', 'admin');
	...
	$backup_file = 'backup/'. str_replace(PHP168_PATH, '', $file) . '.'. $time .'.css';
	$dir = dirname($backup_file);		
	md(PHP168_PATH .'/skin/'. $dir);
	cp($file, PHP168_PATH .'/skin/'. $backup_file);
