...
if(REQUEST_METHOD == 'GET'){
	...
	$sites_enable = isset($systems['sites']) && $systems['sites']['installed'] && $systems['sites']['enabled'];
	...
	case 'all':
		...
		function ___poster($message = ''){
			...
			global $this_url, $core, $_ALLCACHE,$P8LANG;
			$message = $message ? $message : $P8LANG['running'];
			$form = <<<FORM
$message
...
FORM;
            ...
            $read_data = $core->CACHE->read('', 'core', 'cache_lock', 'serialize');
            /*读数据为空时，尝试修改权限*/
            if(empty($read_data)){
                @chmod(CACHE_PATH, 0777);
                @chmod(CACHE_PATH.'core', 0777);
                $core->CACHE->write('', 'core', 'cache_lock', $_ALLCACHE, 'serialize');
                @chmod(CACHE_PATH.'core/core_cache_lock.php', 0766);
            }
			message($form);
		}
		
		function _post_module($sys, $mod){
			global $core, $_ALLCACHE,$P8LANG;
			$message = $P8LANG['running'];
			$form = <<<FORM
$message
...
FORM;
		...
		if(empty($_POST['start'])){
			//读锁
			if($_ALLCACHE){
				message('caching_lock','',3,$core->admin_controller);
			}
