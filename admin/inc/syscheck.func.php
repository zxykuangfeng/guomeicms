...
function get_dirfile_items($dir){
	$dir = clear_special_char($dir);
	$dir = str_replace('.//','./',$dir);
	$scandir = PHP168_PATH.str_replace(array('./','../','/..','//'),'',$dir);
	...
	$scandir = p8_filter_special_chars($scandir);
	...
	foreach($dir_array as $key => $item){
		...
		$dirfile_items[$item] = array(
			...
			'path' => $path,
			'authcode_path' => authcode_token($path,'INCODE',0,true),
...
function dirfile_check(&$dirfile_items){
	foreach($dirfile_items as $key => $item){
		$path = PHP168_PATH.substr($item['path'],1);
		if(file_exists($path)){
			$info = stat($path);
			...
			if($item['type'] == 'file'){
				...
				$dirfile_items[$key]['size'] = sizeformat($info['size']);
			}else{
