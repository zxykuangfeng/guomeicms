...
function connect(){
	...
	if($this->isnew())return $this->open();
...
function open(){
    ...
    try{	
    $this->link = new PDO('mysql:host='.$_host.';dbname='.$this->db, $this->user, $this->password,array(PDO::ATTR_PERSISTENT => $this->pconnect));
...
function query($query, $type = ''){
    ...
    if($this->isnew())return $this->execute($query);
	if(!($query = $this->parsesql($query)))return false;
	//echo "$query<br />\r\n";
	$this->query_num++;
    ...
    try{
        ...
        $this->bindParams($params);
         if(defined('SQL_DEBUG')){
            ...
            fputs($fp, date('Y-m-d H:i:s', P8_TIME) ."\t". $querystr ."\r\n");
            fclose($fp);
        }
       ...
       if(empty($core->CONFIG['debug']) && function_exists('exception_handler')){
           exception_handler($e,'sql_', $query);
           return false;
       }
		echo '<font color=red>SQL ERROR:</font>'.$query."<br>\r\n<pre>";
		echo $e->getCode();
...
function fetch_array($query, $type = ''){
    if($this->isnew())return $this->fetchArray($query);
    $type= $type?:MYSQL_ASSOC;
...
function fetch_one($query){
    if($this->isnew())return $this->fetchone($query);
...
function affected_rows() {
    if($this->isnew())return $this->rowCount();
...
function insert_id(){
    if($this->isnew())return $this->link->lastInsertId();
...
function errno(){
	return $this->isnew()?$this->link->errorCode():mysql_errno($this->link);
}
...
function escape_string($s){
    ...
    if($this->isnew())return $this->escapestring($s);
...
function version(){
	...
	if(!$this->connected) $this->connect();
	$this->version = $this->isnew()?$this->link->getAttribute(PDO::ATTR_SERVER_VERSION):@mysql_get_server_info($this->link);
	return $this->version;
}
...
function checkTableExists($table){
    return $this->fetch_all("SHOW TABLES LIKE '{$table}'");
}

function checkFileExists($table, $file){
    return $this->fetch_one("SHOW FIELDS FROM $table LIKE '$file'");
}

function checkPartition(){
    $partition = $this->fetch_one("SHOW VARIABLES LIKE 'have_partitioning'");

    return empty($partition) || strtolower($partition['Value']) != 'yes' ? false : true;
}
...
function getTableStatus($table){
    return $this->fetch_all("SHOW TABLE STATUS". ($table ? " LIKE '$table'" : ''));
}
...
function getDDL($table){
    return $this->fetch_one("SHOW CREATE TABLE `{$table}`");
}

function formatTable($sql){
    return $sql;
}
...
function formatSql($sqls,$drop_table=false){
    ...
    if($drop_table){
        foreach($sqls as $v){
            if(preg_match("/CREATE TABLE(\sIF NOT EXISTS)? (?<table>[a-z0-9\_\-`.]+).+?\s/i", $v, $m))
                array_unshift($sqls, "DROP TABLE IF EXISTS {$m['table']};");
        }
    }
    return $sqls;
}
...
function showCharacterSet(){
    ...
    while($arr = $this->fetch_array($query)){
        $charsets[] = $arr['Charset'];
    }
    return $charsets;
}

function getDataSize(){
     $query = $this->fetch_one("select sum(data_length) as data from information_schema.tables where  table_schema='{$this->db}'");
        return $query['data'];
}
