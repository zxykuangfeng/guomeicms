	...
	*/
	...
	if(!empty($labels)){
		$head .= "{$rn}\$LABEL = &\$core->load_module('label');{$rn}global \$__label;{$rn}".
			($sub_compile_template || !$file ? '' : 'if(!isset($LABEL_POSTFIX))global $LABEL_POSTFIX;'.$rn.' if(!$SYSTEM)global $SYSTEM; if(!$MODULE)global $MODULE; if(!$SITENAME)global $SITENAME; if(!$ENV)global $ENV; if(!$LABEL_PAGE)global $LABEL_PAGE; $LABEL->init($SYSTEM, $MODULE, $LABEL_PAGE, $SITENAME, $ENV);'. $rn.
	...
	$check_label = '';
	if(!empty($labels)){
		$check_label = "\$__label_empty = true;{$rn}foreach(\$__label as \$__label_key => \$__label_value){{$rn}if(\$__label_value) {\$__label_empty = false;break;}{$rn}}{$rn}if(\$__label_empty){{$rn}\$LABEL->cache();{$rn}\$LABEL->cache_data();".
			"\$LABEL->postfix(isset(\$LABEL_POSTFIX) ? \$LABEL_POSTFIX : array());".
			"{$rn}\$LABEL->get_data_cache();{$rn}".$_labels."}";
	}
	$reflash = !empty($labels) && !$sub_compile_template ? "{$rn}\$LABEL->refresh_labels();{$rn}" : "";
	$head .= implode('', $pages) .$_labels .$check_label .$reflash;
	/** 处理标签} **/
