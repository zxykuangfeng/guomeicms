...
function __construct(){
	...
	$this->RESOURCE_VICE = empty($this->CONFIG['resource_url_vice']) ? $this->RESOURCE : $this->CONFIG['resource_url_vice'];
	$this->STATIC_URL = !empty($this->CONFIG['static_enable']) ? (empty($this->CONFIG['static_url']) ? $this->url : rtrim($this->CONFIG['static_url'],'/')) : $this->url;
	...
	$this->attachment_url = $this->get_attachment_url() .'/'. $this->CONFIG['attachment']['path'];
...
function get_attachment_url(){
    $url = $this->RESOURCE;
    switch ($this->CONFIG['attachment_storate_type']){
        case 'resource_url':
            $url = $this->RESOURCE;
            break;
		case 'resource_url_vice':
            $url = $this->RESOURCE_VICE;
            break;
        case 'url':
            $url =  $this->CONFIG['url'];
            break;
        case 'static_attachment_storage':
            $url =  $this->CONFIG['static_attachment_storage'];
            break;
        case 'static_attachment_url':
            $url =  $this->CONFIG['static_attachment_url'];
            break;
		case 'relative_url':
            $url =  '';
            break;
        case 'ftp_remote':
            $remote_att = $this->CONFIG['attachment']['remote'];
            $url = $remote_att['server'][$remote_att['current']];
            break;
    }
    return rtrim($url,'/');
}
...
function _set_config(
	...
	global $_syn_configs,$STATIC_URL;
	...
	$this->DB_master->insert(
		...
		array(
			...
			'unique' => array('system' ,'module' ,'type' ,'k'),
			'replace' => true
		)
	);
	...
	if($system == 'core' && $cache_name == 'core'){
		$datas['statistics'] = str_replace('{$STATIC_URL}',$STATIC_URL,$datas['statistics']);
		if(empty($datas['system&module'])){
			$_config = $this->CACHE->read('', 'core', '');
			$datas['system&module'] = $_config['system&module'];			
		}
		if(defined('INSTALLING') or $datas['system&module']) $this->CACHE->write($cache_path, $cache_name, '', $datas);
	}else{
...
/**
* (分页)列表
* @param object $select MysqlSelect
* @param int $page 页数
* @param int $page_size 每页的条数
* @param int $count 查找的记录条数
* @param int $start_num 从第几条数据开始，默认是0，不存在偏移量。
* @return array 结果集
**/
function list_item(&$select, $options = array()){
	...
	$options['start_num'] = empty($options['start_num']) ? 0 : intval($options['start_num']);
	...
	if(!empty($options['sphinx']['enabled'])){
		if($list_page){
			//如果需要分页
			$select->limit(($options['page'] -1) * $options['page_size'] + $options['start_num'], $options['page_size']);
...
function check_admin_action($action, $postfix = ''){
	//如果是创始人直接返回
	global $IS_FOUNDER;	
	if($this->core->CONFIG['canot_addable']){
		$candyActions=array('add','update','comment','post','reply','edit');
		if(in_array($action,$candyActions))return false;
	}
...
function file_read($file, $type = 'var_export'){
	...
	//从文件里读缓存
	if($type == 'var_export'){
        if(!empty($this->core->CONFIG['debug']))
            $data = @include $file;
        else
            $data = @include $file;
	}else{
