...
define('API_ROOT',realpath(UC_CLIENT_ROOT . '../').'/');
...
* 3:用户中心子系统
*/
$_setting = setting('member');
define('UC_SERVER', 3);
//相对于用户中心的应用id
define('UC_APPID', $_setting['pw_appid']);
define('UC_CONNECT_METHOD',$_setting['pw_connect']);
define('UC_DBHOST', $_setting['pw_dbhost']);
define('UC_DBUSER', $_setting['pw_dbuser']);
define('UC_DBPW', $_setting['pw_dbpw']);
define('UC_DBNAME', $_setting['pw_dbname']);
define('UC_DBPRE', $_setting['pw_dbtablepre']);
define('UC_DBPCONNECT', $_setting['pw_dbconnect']);
define('UC_DBCHARSET', $_setting['pw_dbcharset']);
...
function uc_user_checkname($username) {
	return uc_data_request('user', 'checkName', array($username));
}
...
function uc_user_checkemail($email) {
	return uc_data_request('user', 'checkEmail', array($email));
}
