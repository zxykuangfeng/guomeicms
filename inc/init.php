...
function write_file($file, $data, $mode = 'wb', $lock = true){
	...
	$fp = @fopen($file, $mode);
	if(!$fp) return false;
	$data = empty($data) ? "<!DOCTYPE html><html><head><meta charset=utf-8><title>".$P8LANG['error']."</title></head><body>".$P8LANG['write_file_error']."</body></html>" : $data;
...
function file_ext($file){
	$_ext = trim(substr(strrchr($file, '.'), 1));
	return strpos($_ext,'/') == FALSE ? $_ext : '';
}
...
function real_path($path, $in_path = ''){
	$path = clean_path(valid_path(realpath($path)));
...
function html_entities2($string) {
	global $core;
	$charset = (empty($core->CONFIG['page_charset']) || $core->CONFIG['page_charset']=='gbk')? 'ISO-8859-1':$core->CONFIG['page_charset'];
...
function xss_url($url){
	...
	$temp = preg_replace_callback('/(\\\X\w{2})/','hexToStr',$temp);
	$temp = preg_replace('/<\w+(.*?)>(.*?)<\/\w+>/','',$temp);
	$temp = urlencode($temp);
	$temp = str_replace(array('%22','%27','%3E','%3C','%3D','%26','%3F','%2F','%3A','%23','JAVASCRIPT','SCRIPT'),array('','','','','=','&','?','/',':','#','SB','SB'),$temp);
...
function hexToStr($hex){
	return chr(hexdec($hex[0]));
}

function clear_special_char($str,$exp_dot = false){
    $regex = "/\.\.\/|\.\.\\|\.\/|\.\\|\~|\!|\@|\#|\\$|\%|\^|\&|\*|\(|\)|\+|\{|\}|\:|\<|\>|\?|\[|\]|\,|\.|\;|\'|\`|\=|\\\|\|/";
    if($exp_dot) $regex = "/\.\.\/|\.\.\\|\.\/|\.\\|\~|\!|\@|\#|\\$|\%|\^|\&|\*|\(|\)|\+|\{|\}|\<|\>|\?|\[|\]|\,|\;|\'|\`|\=|\\\|\|/";    
...
function generate_unique_key($inputString) {
    $hash = hash('sha256', $inputString);    
    $base36 = base_convert($hash, 16, 36);    
    $uniqueString = substr($base36, 0, 16);    
    return $uniqueString;
}
function p8_html_filter_array($array){
    if(!is_array($array)){
        return p8_html_filter($array);
    }
    
    foreach ($array as $key=>$arr){
        if(is_array($arr)){
            $array[$key]=p8_html_filter_array($arr);
        }else{
			$array[$key]=p8_html_filter($arr);
		}
    }
    return $array;
}
...
function p8_cutstr($string, $length, $dot = '...') {
	...
	$_string = '';
	$useMbstring = function_exists('mb_strlen'); // 检查mbstring函数是否存在
    if ($useMbstring && mb_strlen($string, 'UTF-8') <= $length) {
        return $string;
    }
	if($useMbstring) {
        // 使用mbstring函数处理UTF-8编码的字符串
        $_string = mb_substr($string, 0, $length, 'UTF-8');
        // 检查最后一个字符是否是多字节字符的一部分
        if (mb_strlen($_string, 'UTF-8') > $length) {
            $_string = mb_substr($_string, 0, mb_strlen($_string, 'UTF-8') - 1, 'UTF-8');
        }
    } else if(strtolower($core->CONFIG['page_charset']) == 'utf-8') {
		// 如果mbstring不可用，使用原始逻辑处理字符串
		$n = $tn = $noc = 0;
		while($n < $len) {
...
function init_session(){
	...
	//if(empty($P8SESSION)){
		...
		if($core->DB_master->insert($core->TABLE_ .'session', $P8SESSION)){
			...
			//session fail to do this
			$session_table = $core->TABLE_ .'session';
			delete_session('lastview < \''. (P8_TIME - 86400) .'\'');
			delete_session('uid=0 AND lastview < \''. (P8_TIME - 1200) .'\'');
			$core->DB_master->query("REPAIR TABLE $session_table;");
			$core->DB_master->query("OPTIMIZE TABLE $session_table");
			$_P8SESSION = array('_hash' => $hash);
			$P8SESSION['data'] = &$_P8SESSION;
			$P8SESSION['data1'] = $P8SESSION['data2'] = $P8SESSION['data3'] = $P8SESSION['data4'] = '';
			if($core->CACHE->memcache){
...
* @$expiry：密文有效期
**/ 
function authcode_token($string, $operation = 'DECODE', $expiry = 7200,$no_special_chars = false) {	
	global $_P8SESSION,$core;
	if($no_special_chars && $operation == 'DECODE'){
		$replace_array = array(
			'-1-' => '%',
			'-2-' => '#',
			'-3-' => '&',
		...
		$string = str_replace(array_keys($replace_array), $replace_array, $string);		
	}
	$expire = isset($core->CONFIG['expire'])?intval($core->CONFIG['expire'])*60:0;
	$expiry = $expire>=0 ? $expire : 7200;
	$ckey_length = 4;
	$key = md5('php168');
	...
	if($operation == 'DECODE') {
		if((substr($result, 0, 10) == 0 || substr($result, 0, 10) - time() > 0) && substr($result, 10, 16) == substr(md5(substr($result, 26).$keyb), 0, 16)) {
		   $encode = substr($result, 26);
		   if($encode && substr($encode,3,strlen($_P8SESSION['_hash'])) == $_P8SESSION['_hash']) {
			   unset($_P8SESSION['csrf_token']);			   
			   return $encode;
		   }else{
			   return '';
		   }
		...
		$token = $keyc.str_replace('=', '', base64_encode($result));
		$_P8SESSION['csrf_token'] = $token;
		if($no_special_chars){
			$replace_array = array(
				'-1-' => '%',
				'-2-' => '#',
			...
			$replace_array = array_flip($replace_array);
			$token = str_replace(array_keys($replace_array), $replace_array, $token);		
		}
		return $token;
	}
}
 ...
 * 过滤 .\ 和 ..\
**/
function p8_filter_special_chars($inputPath) {    
 ...
 */
function valid_word_scan($content){
	global $core, $CACHE,$P8LANG;
	
	static $cache;
	static $cache_right_word;
	if($cache === null){
		$cache = $CACHE->read('', $core->name, 'word_scan');
	}
	if((!$cache && !$config['scan_word_group']) || !$content){
		return array();
	}
	$config = $core->get_config('core','');
	$scan_word_group = $noscan_word_group = array();
	$filter = $comma = '';
	foreach($config['scan_word_group'] as $group){
		$scan_word_group[] = $group['scan_word'];
		$noscan_word_group[] = $group['noscan_word'];
		$filter .= $comma . $group['scan_word'];
		$comma = '|';
	}	
	if($cache){
	...
	if($matches && $config['scan_word_group']){		
		foreach($matches as $keys=>$word){
			if(in_array($word,$scan_word_group)){
				$all_strpos = get_filter_word_pos($word,$content);
				$index = array_keys($scan_word_group,$word);
				$index = $index[0];
				$_check = false;
				foreach($all_strpos as $pos){
					$star = $pos - abs(strlen($noscan_word_group[$index]) - strlen($scan_word_group[$index]));
					$length = max(strlen($noscan_word_group[$index]),strlen($scan_word_group[$index])) + abs(strlen($noscan_word_group[$index]) - strlen($scan_word_group[$index]));
					$get_str = substr($content,$star<=0?0:$star,$length);
					//是否通过忽略检测
					$_check = strpos($get_str,$noscan_word_group[$index]) === FALSE ? false : true;
					if(!$_check){
						$matches[$keys] = $word.'(<font color="#0000FF">'.$P8LANG['the_no_filter_word_word'].':'.$noscan_word_group[$index].'</font>)';
						break;
					}
				}
	...
	//正确词语提示
	if(count($matches)){
		$cache_right_word = $CACHE->read('', $core->name, 'word_scan_right');
		if($cache_right_word){
			foreach($matches as $key=>$temp_match){
				$index = array_search($temp_match,$cache_right_word['filter_word']);
				if($index !== FALSE){
					$matches[$key] .= isset($cache_right_word['right_word'][$index]) && $cache_right_word['right_word'][$index] ? '(<font color="#0000FF">'.$P8LANG['the_right_word'].':'.$cache_right_word['right_word'][$index].'</font>)' : '';
				}
			}
		}
	}
...
* @param array 消息中的参数
* @goback string 是否有返回
 * @s2 string 在传递$s为数组的情况下，输入的提示语
* 如参数$s 为 "%s, 你好, 现在是 %s", $params 的参数为 arrray('某人', '2010年')
* 那么输出信息为 "某人, 你好, 现在是 2010年"
**/
function message($s, $forward = '', $timeout = 3, $params = array(),$goback=1,$s2=''){
	global $P8LANG,$ISBACK, $core, $RESOURCE;
...
* @return string
**/
function p8_url(&$M, &$data, $action = 'view', $first_page = true, $ismobile = false){
	global $core;
	...
	//Return to original state
 	if($ismobile) $core->ismobile = $temp;	
	return $url;
}
...
function list_page($param){
    ...
    $site = isset($param['site']) ? $param['site'] : '';
	...
	if(preg_match('/\{first_page:([^\}]+)(?:\|default:)([^\}]+)\}/', $template, $m)){
		...
		if($cid) $link = str_replace('?cid?', $cid, $link);
        if($site) $link = str_replace('?site?', $site, $link);
...
function &select(){
	...
	global $core;
    $className = ucfirst($core->CONFIG['mysql_connect_type']).'Select';
    if(file_exists(PHP168_PATH.'inc/select/'.$className.'.php')){
        require_once PHP168_PATH.'inc/select/'.$className.'.php';
...
function uploaded_attachments(&$obj, $item_id, $hash){
	...
	$site =  get_cookie($key) !== null ? basename(get_cookie($key)) : '';
	$data = array('item_id' => $item_id);
	if($site && strpos($table, 'sites_attachment') !== false){
		$data = array(
			'item_id' => $item_id,
			'site' => $site
		);
...
**/
function attachment_url($content, $confuse = false,$force = false){
	
	global $core;
	
	$config = &$core->CONFIG['attachment'];
	//开关
	//if(empty($force) && empty($config['confuse'])) return $content;
	...
	if(is_array($content)){
		foreach($content as $k => $v) $content[$k] = attachment_url($v, $confuse, $force);
...
function cc_attack(){
    ...
    $hash = P8_IP.'#'.md5($_SERVER[HTTP_USER_AGENT]);
	$cc_attack_time = 3;
	...
	if($limit_time < 0){
		...
		if(strstr($cc_attack_ip_file,$hash) && !strstr(REQUEST_URI,'-cache')){
	...
	if(P8_TIME - @filemtime(CACHE_PATH.'cc_attack.txt') > $cc_attack_time){
		...
		foreach($detail AS $value){	
			...
			if($_detail[$value] > $cc_attack_num){
				write_file(CACHE_PATH.'cc_attack_ip.txt',P8_TIME.' '.$hash."\r\n",'a');
			}
		}
	}
    if( !strstr(REQUEST_URI,'-cache'))write_file(CACHE_PATH.'cc_attack.txt',$hash."\r\n",'a');
...
function stop_ip(){
	...
	//ip段,支持*号
	if(!empty($config['stop_ip']['collectip'])){
		$ipregexp = implode('|', str_replace(array('*','.'), array('\d+','\.') ,array_flip($config['stop_ip']['collectip'])));
		if(preg_match("/^(".$ipregexp.")$/", P8_IP)) die( "Forbid Ip!!" );
	}
    ...
    if(!empty($config['stop_ip']['area_ip'])){
        foreach($config['stop_ip']['area_ip'] as $ipss=>$k){
            ...
            if(!empty($ippre_begin) && !empty($ippre_end) && $ippre_begin == $ippre_end){
                ...
                if($ippre_end == $ippre_user && intval(substr(P8_IP, $pos_user+1)) >= intval(substr($pos_begin, $pos_begin+1)) && intval(substr(P8_IP, $pos_user+1)) <= intval(substr($pos_end, $pos_end+1))){
                    die( "Forbid Ip!!" );
                }
            }
...
function allow_ip(){
	...
	if(!empty($config['allow_ip']['collectip'])){
		$ipregexp = implode('|', str_replace(array('*','.'), array('\d+','\.') ,array_flip($config['allow_ip']['collectip'])));
 ...
 * 后台允许IP
 */
function allow_admin_ip(){
	global $core;
	$config = &$core->CONFIG;
	
	if(!isset($config['allow_admin_ip']['enabled']) || $config['allow_admin_ip']['enabled'] == 0){
		return false;
	}
    if($config['allow_admin_ip']['enabled'] == 2){
		return area_ip();
	}
	
	//ip集合序列
	if(isset($config['allow_admin_ip']['collectip'][P8_IP])){
		return true;
	}
	//允许的ip段,支持*号
	if(!empty($config['allow_admin_ip']['collectip'])){
		$ipregexp = implode('|', str_replace(array('*','.'), array('\d+','\.') ,array_flip($config['allow_admin_ip']['collectip'])));
		if(preg_match("/^(".$ipregexp.")$/", P8_IP)) return true;
	}
	
	//ip段	
	if(!empty($config['allow_admin_ip']['beginip']) && !empty($config['allow_admin_ip']['endip'])){
		
		$pos_begin = strrpos($config['allow_admin_ip']['beginip'], '.');
		$pos_end = strrpos($config['allow_admin_ip']['endip'], '.');
		$pos_user = strrpos(P8_IP, '.');
		
		$ippre_begin = ($pos_begin === false) ? '' : substr($config['allow_admin_ip']['beginip'], 0, $pos_begin) ;
		$ippre_end = ($pos_end === false) ? '' : substr($config['allow_admin_ip']['endip'], 0, $pos_end);
		$ippre_user = ($pos_user === false) ? '' : substr(P8_IP, 0, $pos_user);
		...
		if(!empty($ippre_begin) && !empty($ippre_end) && $ippre_begin == $ippre_end){
			...
			if($ippre_end == $ippre_user && intval(substr(P8_IP, $pos_user+1)) >= intval(substr($config['allow_admin_ip']['beginip'], $pos_begin+1)) && intval(substr(P8_IP, $pos_user+1)) <= intval(substr($config['allow_admin_ip']['endip'], $pos_end+1))){
				//ip例外
				if(!isset($config['allow_admin_ip']['ruleoutip'][P8_IP])){
					return true;
				}
			}
		}
...
function area_ip(){
	...
	$config = &$core->CONFIG;
	//没填写特殊IP允许，则全允许，不再检测
	$area_ip = $collectip = array('0'=>0);
	if($config['cate_ip']['collectip']){
		$collectip = array_flip($config['cate_ip']['collectip']);
	}
	if($config['cate_ip']['area_ip']){
		$area_ip = array_flip($config['cate_ip']['area_ip']);
	}
	if(empty($collectip[0]) && empty($area_ip[0])) return true;
	
	//ip集合序列
	if(isset($config['cate_ip']['collectip'][P8_IP])){
		return true;
	}
	
	//允许的ip段,支持*号
	if(!empty($config['cate_ip']['collectip'])){
		$ipregexp = implode('|', str_replace(array('*','.'), array('\d+','\.') ,array_flip($config['cate_ip']['collectip'])));
		if(preg_match("/^(".$ipregexp.")$/", P8_IP)) return true;
	}

	//ip段
	if(!empty($config['cate_ip']['area_ip'])){
        foreach($config['cate_ip']['area_ip'] as $ipss=>$k){
        list($beginip,$endip)=explode('-',$ipss);
		$pos_begin = strrpos($beginip, '.');
		$pos_end = strrpos($endip, '.');
		$pos_user = strrpos(P8_IP, '.');
		...
		if(!empty($ippre_begin) && !empty($ippre_end) && $ippre_begin == $ippre_end){
			...
			if($ippre_end == $ippre_user && intval(substr(P8_IP, $pos_user+1)) >= intval(substr($beginip, $pos_begin+1)) && intval(substr(P8_IP, $pos_user+1)) <= intval(substr($endip, $pos_end+1))){
				//ip例外
				if(!isset($config['cate_ip']['ruleoutip'][P8_IP])){
					return true;
				}
			}
		}
...
function front_ip(){
	global $core;
	$config = &$core->CONFIG;
	if(!isset($config['front_ip']['enabled']) || $config['front_ip']['enabled'] == 0){
		return false;
	}
	if($config['front_ip']['enabled'] == 2){
		return area_ip();
	}
	//ip集合序列
	if(isset($config['front_ip']['collectip'][P8_IP])){
		return true;
	}
	
	//允许的ip段,支持*号
	if(!empty($config['front_ip']['collectip'])){
		$ipregexp = implode('|', str_replace(array('*','.'), array('\d+','\.') ,array_flip($config['front_ip']['collectip'])));
		if(preg_match("/^(".$ipregexp.")$/", P8_IP)) return true;
	}

	//ip段
	if(!empty($config['front_ip']['area_ip'])){
        foreach($config['front_ip']['area_ip'] as $ipss=>$k){
        list($beginip,$endip)=explode('-',$ipss);
		$pos_begin = strrpos($beginip, '.');
		$pos_end = strrpos($endip, '.');
		$pos_user = strrpos(P8_IP, '.');
		...
		if(!empty($ippre_begin) && !empty($ippre_end) && $ippre_begin == $ippre_end){
			...
			if($ippre_end == $ippre_user && intval(substr(P8_IP, $pos_user+1)) >= intval(substr($beginip, $pos_begin+1)) && intval(substr(P8_IP, $pos_user+1)) <= intval(substr($endip, $pos_end+1))){
				//ip例外
				if(!isset($config['front_ip']['ruleoutip'][P8_IP])){
					return true;
				}
			}
		}
...
function u_ip(){
	global $core;
	$config = &$core->CONFIG;
	if(!isset($config['u_ip']['enabled']) || $config['u_ip']['enabled'] == 0){
		return false;
	}
	if($config['u_ip']['enabled'] == 2){
		return area_ip();
	}
	//ip集合序列
	if(isset($config['u_ip']['collectip'][P8_IP])){
		return true;
	}
	
	//允许的ip段,支持*号
	if(!empty($config['u_ip']['collectip'])){
		$ipregexp = implode('|', str_replace(array('*','.'), array('\d+','\.') ,array_flip($config['u_ip']['collectip'])));
		if(preg_match("/^(".$ipregexp.")$/", P8_IP)) return true;
	}

	//ip段
	if(!empty($config['u_ip']['area_ip'])){
        foreach($config['u_ip']['area_ip'] as $ipss=>$k){
        list($beginip,$endip)=explode('-',$ipss);
		$pos_begin = strrpos($beginip, '.');
		$pos_end = strrpos($endip, '.');
		$pos_user = strrpos(P8_IP, '.');
		...
		if(!empty($ippre_begin) && !empty($ippre_end) && $ippre_begin == $ippre_end){
			...
			if($ippre_end == $ippre_user && intval(substr(P8_IP, $pos_user+1)) >= intval(substr($beginip, $pos_begin+1)) && intval(substr(P8_IP, $pos_user+1)) <= intval(substr($endip, $pos_end+1))){
				//ip例外
				if(!isset($config['u_ip']['ruleoutip'][P8_IP])){
					return true;
				}
			}
		}
...
function filter_word($content, $replacement = '', $test = false){
	...
	$config = $core->get_config('core','');
	if(!$cache || !$content || !$config['filter_word_enable']){
		return $content;
	}
...
function get_member($username, $refresh = false, $type = 'username'){
	...
	$where = (in_array($type,array('username','email','id','cell_phone'))? $type : 'username') . " = '$username'";
...
function is_front(){
    global $core,$FROMURL;
    $detail = parse_url($FROMURL);
    $detail2 = explode('/',ltrim($detail['path'],'/'),2);
    return in_array($detail2[0],array('u.php','admin.php',$core->CONFIG['admin_controller']));
}
...
function exception_handler($exception, $pix='php_', $momsg='') {
    global $core;
    $string = $exception->getMessage().' In File ' .$exception->getFile().' Line:' .$exception->getLine()."\r\n".($momsg?'MESSAGE: '.$momsg."\r\n":'') .$exception->getTraceAsString ();
    if(!empty($core->CONFIG['debug'])){
        echo ':(<br/>'.nl2br($string);
    }else{
        @file_put_contents(CACHE_PATH.'runtime/'.$pix.'error_'.date('Y-m-d').'.php',
        "\r\n[".date('Y-m-d H:i:s',P8_TIME)."]\r\n".$string,
        FILE_APPEND
        );
    }
}
/*
*apache_request_headers不存在时的处理
*/
...
//请求方式,get,post
define('REQUEST_METHOD', $ISCLI?'CLI':strtoupper($_SERVER['REQUEST_METHOD']));
...
//是否开启魔法引号
define('MAGIC_QUOTES', phpversion()<8? get_magic_quotes_gpc():false);
...
preg_match('/[\d\.]{7,15}/', $onlineip2, $onlineipmatches2);
$onlineip2 = isset($onlineipmatches2[0]) ? $onlineipmatches2[0] : 'unknown';
//IP
