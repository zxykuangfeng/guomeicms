...
function get_install_sql(&$db, $sql, $table_prefix, $drop_table = false){
	...
	$sql = preg_replace("/(type|engine)\s*?=\s*?([a-z0-9]+)/i", 'ENGINE=MyISAM'. $default_charset, $sql);	//添加字符集
	//分割每个SQL
	$sql = str_replace([";\r","-- <?php exit;?>"], [";\n",''], $sql);
	//去除空行,转换编码
	...
	return $db->formatSql($sql,$drop_table);
}
...
function to_installed_charset($str){
	global $core;
	if('utf-8' == $core->CONFIG['page_charset']) return $str;	//编码相同不鸟
	
	return convert_encode('utf-8', $core->CONFIG['page_charset'], $str);
}
...
function installog($system, $module, $step, $msg=''){
    $line = sprintf("system:%s, module:%s, step:%s msg:%s", $system,$module,$step,$msg);
    wlog('install.log',$line);
}
