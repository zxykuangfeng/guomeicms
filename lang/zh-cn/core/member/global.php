<?php

return array(
	
	'no_such_username' => '错误,错误代码1',
	'user_no_verify' => '此用户还在审核中',
	'user_locking' => '此用户处于锁定状态',
	'username' => '用户名',
	'usericon' => '用户头像',
	'username_length_range' => '用户名必须是3 - 20个字符',
	'username_required' => '用户名必须填写',
	'username_invalid' => '用户名不允许有特殊字符',
	'username_exists_or_denied' => '用户名已存在或用户名不允许注册',
	'username_hint' => '支持中英文注册',
	'mail_error' => '邮箱错误！',
	'role_group_field_required' => '此字段必须填写',
	'confirm_to_logout' => '确认退出吗？',
	'confirm_to_delete_member' => '确定要删除会员吗?',
	'role_group_default_role' => '角色组默认角色',
	'login_success' => '登录成功, <a href="{$1}" style="color:blue;">进入会员中心>></a> | <a href="{$2}" style="color:blue;">进入首页>></a>',
	'profile' => '会员资料',
	'confirm_password' => '确认密码',
	'old_password' => '旧密码',
	'new_password' => '新密码',
	'register_time' => '注册时间',
	'last_login_time' => '上次登录时间',
	'last_login_ip' => '上次登录IP',
	'login_time' => '登录次数',
	'phone' => '电话',
	'cell_phone' => '手机',
	'cell_phone_required' => '手机必填',
	'cell_phone_hint' => '用于接收短信和找回密码',
	'join_contact_list' => '是否加入通讯录',
	'join_contact_list_note' => '通常只设置部门的管理员，慎用。',
	'short_message_user' => '是否短消息默认用户',
	'short_message_user_note' => '在站内短消息时，用户将直接显示在右侧备选。',
	'address' => '地址',
	'role_gname' => '岗位',
	'role_gname2' => '所在组织',
	'role_name' => '角色',
	'status' => '状态',
	'email' => '电子邮箱',
	'address_list' => '部门员工',
	'number' => '身份证',
	'register_with_captcha' => '注册是否需要验证码',
	'register_disabled' => '禁止注册',
	'login_with_captcha' => '登录是否需要验证码',
	'register_success' => '账号注册成功！',
	'verify_question' => '验证问题',
	'findpwd_disabled' => '禁止用户自行重置密码，请联系管理员！',
	'register_question_enabled' => '是否开启注册问题',
	'select_dept' => '请选择所在组织',
	'verify_question_required' => '验证问题必须填写',
	'verify_question_incorrect' => '验证问题不正确',
	'role_select' => '选择注册角色',
	'role_register' => '注册角色',
	'role_required' => '注册角色必选',
	'email_required' => 'E-mail 必须填写',
	'email_invalid' => 'E-mail 格式不正确',
	'email_registerd' => 'E-mail 已被注册',
	'nextstep' => '下一步',
	'question' => '密保问题',
	'find_pwd_noset' => '没有设置密保问题，无法重置，请联系管理员',
	'find_pwd_fail' => '很抱歉，密保问题答对数量必须大于2个',
	'reset_pwd_fail2' => '认证失败',
	'reset_pwd_fail' => '重置失败，请重新输入密码',
	'change_password' => '修改密码',
	'password_required' => '密码必须填写',
	'password_length_range' => '密码必须是10 - 20个字符',
	'password_strong' => '大写、小写、数字、特殊符号(特殊符号只能配置一位)，位数不少于10位',
	'password_not_match' => '两次密码不匹配',
	'pw_level_too_low' => '密码过于简单，需大写、小写、数字、特殊符等组成，不少于8位',
	'phone_invalid' => '手机号码不正确',
	'phone_too_long' => '手机号码太长',
	
	'agreement_required' => '必须同意注册协议',
	
	'wrong_password' => '错误,错误代码1',
	'address_list' => '通讯录',
	'request_to_be_friends' => '{$1}请求加你为好友',
	'request_to_be_friends_content' => '{$1}请求加你为好友。<br />{$2}<br />验证<a href="'. $core->U_controller .'/member-friend?verified=0"><font color="red">传送门</a></a>',
	'verified_to_be_friends' => '{$1}通过了你的好友请求',
	'to_be_your_friend' => '{$1}加你为好友',
	
	'cache_acl_init' => '会员权限缓存初始化',
	'cache_acl_success' => '会员权限缓存完毕',
	'cache_acl' => '会员权限权限缓存',
	'cache_member' => '会员缓存',
	
	'member_status' => array(
		'' => '会员状态',	
		0 => '正常',
		1 => '待审核',
		2 => '锁定',
	),
	'member_status_3' => '3个月没登录',
	'member_status_6' => '6个月没登录',
	'member_status_12' => '12个月没登录',
	'recharge' => array(
		'' => '充值',
		'order_name' => '充值 {$1}',
		'done' => '充值成功',
		'not_paid' => '未付款',
		'confirm_note' => '确认之前请确定已经收到买家的付款',
		
		'done_message' => '充值成功!',
		'done_message_content' => '你充值的{$1} {$2} {$3}已经完成。请查收。',
		
		'select_amount' => '选择充值金额',
		'note' => '1、选择金额：请选择您需要的充值的金额。<br />
		2、支付状态：您对此笔充值情况，如还没有充值成功，可以点击“充值管理”来完成这次支付<br />
		3、详细说明：充值的币种可以根据网站需要，支付不同商品购买，包括广告。<br />',
	),
	
	'buy_role' => array(
		'' => '购买会员权限',
		'select_role' => '选择需要升级的会员组',
		'note' => '<p>1,不同的用户组有不同的权限，请选择自身需要的用户组</p><p>2,你可以通过支会购买方式升级到相应用户组</p><p>3,你可以选择包月、包季度、包年等多种套餐服务</p>',
		'order_name' => '升级到“{$1}”',
		'need_maney' => '需要支付',
		'confirm_note' => '确定之前请确认你已经收到对方付款'
	),
	'register_active_success' => '帐号激活成功，请重新<a href="{$1}">登录</a>',
	'_module_login_u_log' => '登录会员中心',
	'_module_logout_u_log' => '退出会员中心',
);
