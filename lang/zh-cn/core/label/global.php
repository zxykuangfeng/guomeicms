...
return array(
	'exclude' => '不包含',
	'label_alias' => '栏目链接',
	'cms_label_alias' => '主站栏目分类',
	'sites_label_alias' => '子站栏目分类',
	...
	'label_scope_system' => '显示域（系统）',
	'label_scope_module' => '显示域（模块）',
	'label_scope_note' => '<br />如选择了"主站系统",不选模块,那么此标签可以在主站系统下任何页面,包括其所有模块页面调用<br />
	如选择了"主站系统","内容模块",那么此标签只能在 "主站系统的内容模块" 下的模板调用<br />
	如果选择"核心",不选模块,那么全站模板都可以调用这条标签<br />
	如果配置有后缀,可以针对如某个分类ID为99,后缀为category_99,那么就可以针对这个后缀来配置独立的标签,仅在后缀为category_99的模板页面调用;<br />
	如果是供JS调用的标签，必须选择为“无”',
	
	'label_postfix' => '标签后缀',
	'label_postfix_note' => '标签后缀为了标签显示域的命名空间，能使同一显示域有更多的命名空间，此值最好不懂不要修改，如果是供JS调用的标签则不填！',
	...
	'label_template' => '标签模板',
	'label_ttl' => '标签缓存时间',
	'label_ttl_note' => '单位 秒, 缓存周期内将不会读取数据库,配置0为永不更新, -1为即时更新',
