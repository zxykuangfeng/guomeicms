...
* @param int $hour 每天几点执行
* @param int $hour 每隔几小时执行
* 优先由$day,再到$week,最后$month, 如果前面都没有, 则只取$hour
* 如每天0点执行, $day = 1, $week = 0, $month = 0, $hour = 0 $time=0
* 如每星期一0点执行, $day = 0, $week = 1, $month = 0, $hour = 0 $time=0
* 如每月一号0点执行, $day = 0, $week = 0, $month = 1, $hour = 0 $time=0
* 如每60天0点执行, $day = 60, $week = 0, $month = 0, $hour = 0 $time=0
* 如每天23点执行, $day = 0, $week = 0, $month = 0, $hour = 0 $time=23
...
* 
**/
function check_status($script){
	$data = $this->DB_master->fetch_one("SELECT `id`,`status` FROM $this->table WHERE `script` = '$script'");
...
function next_run_time($task){
	...
	if($task['day']){
		...
		//按每天几点执行
		return mktime($task['time'],  $task['minute'], 0, $month, $day+1, $year);
	}else if($task['hour']){
		//按每几小时执行
