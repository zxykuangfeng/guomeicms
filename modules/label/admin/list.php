...
if(REQUEST_METHOD == 'GET'){
	...
	$ttl = $last_update = $update_time = array(null, null);
	...
	if($sitesd){
		...
		foreach($this_sys->sites as $key=>$val){
			if($val['status']){
				$all_sites[$key] = $val;
			}
		}
	...
	$main_url = $core->url ? $core->url : $core->CONFIG['static_url'];
	...
	$select = select();
	$select->from($this_module->table, 'id, system, module, name, type, variable, postfix, timestamp, last_update,update_time,last_setter, ttl, invoke, site');
	...
	if(isset($_GET['update_time'][0]) && strlen($_GET['update_time'][0]) && ($time = strtotime($_GET['update_time'][0]))){
		$update_time[0] = $time;
		$page_url .= '&update_time[0]='. $_GET['update_time'][0];
	}
	if(isset($_GET['update_time'][1]) && strlen($_GET['update_time'][1]) && ($time = strtotime($_GET['update_time'][1]))){
		$update_time[1] = $time;
		$page_url .= '&update_time[1]='. $_GET['update_time'][1];
	}
	...
	if($update_time[0] !== null || $update_time[1] !== null){
		$select->range('update_time', $update_time[0], $update_time[1]);
		$select->order('update_time ASC');
		
		$update_time[0] = @$_GET['update_time'][0];
		$update_time[1] = @$_GET['update_time'][1];
	}
	if($last_update[0] !== null || $last_update[1] !== null){
		$select->range('last_update', $last_update[0], $last_update[1]);
		$select->order('last_update ASC');
		
		$last_update[0] = @$_GET['last_update'][0];
		$last_update[1] = @$_GET['last_update'][1];
	}
	if($error){
		$select->where("system = 'sites' and site = ''");
	}
