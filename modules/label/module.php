...
function __construct(&$system, $name){
	...
	$this->attach_table = $this->core->TABLE_ .'attachment';
		...
		foreach($this->_data as $k => $v){
			if(isset($v[$name])){
				...
				//){
					...
					$ssi = empty($this->_data[$k][$name]['ssi']) ? '' : $this->_data[$k][$name]['ssi'];
				//}else{
				//	$ret = '';
				//}
...
function edit_mode($name){
	...
	$height = isset($label['option']['place_holder_height']) ? $label['option']['place_holder_height'] : 30;
...
function get_cache(){
    ...
    if(empty($this->_scopes)){
        $scopes = array('core','cms','cms-item','cms#index');
        ...
        if(isset($systems['sites'])){
            ...
            $scopes[] = 'site';
            $scopes[] = 'site-item';
            foreach($allsites as $site=>$site_items){
                $scopes[] = 'sites-item='.$site;
                $scopes[] = 'sites='.$site;
            }
        }
    }
    $scopes = !empty($this->_scopes) ? $this->_scopes : $scopes;
    foreach($scopes as $v){
        if(empty($this->_labels[$v]))
            $empty[$v] = 1;
    }

	if(empty($this->_labels) or count($empty)>2){
		//$this->cache();
		$this->cache_data($empty);
	}
	//针对生成静态,暂时保存有后缀的标签
	...
	foreach($this->_scopes as $v){
		...
		if(empty($label_file_read)){
			$label_file_read = $this->core->CACHE->read('label/'.$this->last_label_cache.'/#'. $tmp['system'], $tmp['module'], $tmp['site'].$tmp['env'].$tmp['postfix'] . $this->last_label_cache, 'serialize');			
		}
...
function get_data_cache(){
	...
	foreach($this->_scopes as $v){
		...
		$read_cache_data = $this->core->CACHE->read('label/data/'. $tmp['system'], $tmp['module'], $tmp['site'] .$tmp['env'] . $tmp['postfix'] . $this->last_cache, 'serialize');
		$read_cache_data = $read_cache_data ? $read_cache_data : $this->core->CACHE->read('label/'.$this->last_label_cache.'/data/'. $tmp['system'], $tmp['module'], $tmp['site'] .$tmp['env'] . $tmp['postfix'] . $this->last_cache, 'serialize');
		$this->_data = array_merge(
			array($v => $read_cache_data),
			$this->_data
		);
	...
	//如果数据为空,尝试重新生成
	$isempty = array();
	foreach($this->_scopes as $vv){
		if(empty($this->_data[$vv])){
			$isempty[$vv] = 1;
		}
	}
	if($isempty) $this->cache_data($isempty);
}
...
function fetch(&$label, $var = null){
	...
	case 'module_data':
		...
		if($label['source_system'] == 'core'){
			...
			if($this_system->modules == null){
				/*
				["name"]=> string(4)	"item"
				["url"]=> string(39) "http://localhost/sharp/cms/modules/item" 
				["controller"]=> string(41) "http://localhost/sharp/index.php/cms/item"
				["U_controller"]=> string(37) "http://localhost/sharp/u.php/cms/item"
				["alias"]=> string(18) "主站内容模块"
				["class"]=> string(11) "P8_CMS_Item"
				["controller_class"]=> string(22) "P8_CMS_Item_Controller"
				["installed"]=> bool(true) 
				["enabled"]=> bool(true) 
				*/				
				$this_system->modules[$label['source_module']] = array(
					'name' => $label['source_module'],
					'controller_class' => strtoupper($core->TABLE_.$label['source_system']).'_'.ucwords($label['source_module']).'_Controller',
					'class' => strtoupper($core->TABLE_.$label['source_system']).'_'.ucwords($label['source_module']),
					'installed' => true,
					'enabled' => true,
				);
				$next_module = $label['source_module'] == 'item' ? 'category' : 'item';
				$this_system->modules[$next_module] = array(
					'name' => $next_module,
					'controller_class' => strtoupper($core->TABLE_.$label['source_system']).'_'.ucwords($next_module).'_Controller',
					'class' => strtoupper($core->TABLE_.$label['source_system']).'_'.ucwords($next_module),
					'installed' => true,
					'enabled' => true,
				);
			}
	...
	case 'image':	
		...
		$imgurl = $imgurl? $imgurl: ($label['option']['image']['url'] ? $label['option']['image']['url'] : '');
		$srhs[] = '<!--#p8_attach#-->';
		$srhs[] = '<!--#p8_r_attach1#-->';
		$reps[] = $this->core->attachment_url;
		$reps[] = $this->core->attachment_url;
		if(!empty($label['option']['image']['name']) && substr($label['option']['image']['name'],0,8) == "<!--#p8_"){
			$label['option']['image']['name'] =  attachment_url($label['option']['image']['name']);
			$label['option']['image']['name'] = substr($label['option']['image']['name'],0,8) == "<!--#p8_" ? str_replace($srhs,$reps,$label['option']['image']['name']) : $label['option']['image']['name'];	
		}
		if(empty($imgurl)){
			if($this->core->CONFIG['lazyload']){
				$content = '<img width="' . $label['option']['width'] .'" height="'. $label['option']['height'] .'" border="none" alt="'. $label['option']['image']['title'] .'"  src="'.$RESOURCE.'/images/nopic.jpg" class="lazy" original="'. $label['option']['image']['name'] .'" />';
			}else{
				$content = '<img width="' . $label['option']['width'] .'" height="'. $label['option']['height'] .'" border="none" alt="'. $label['option']['image']['title'] .'" src="'. $label['option']['image']['name'] .'" />';
			}
		}else{
			if($this->core->CONFIG['lazyload']){
				$content = '<a href="'. $imgurl .'" target="_blank" title="'. $label['option']['image']['title'] .'" style="text-decoration:none;">'.
					'<img width="' . $label['option']['width'] .'" height="'. $label['option']['height'] .'" border="none" alt="'. $label['option']['image']['title'] .'" src="'.$RESOURCE.'/images/nopic.jpg" class="lazy" original="'. $label['option']['image']['name'] .'" />'.
				'</a>';
			}else{
				$content = '<a href="'. $imgurl .'" target="_blank" title="'. $label['option']['image']['title'] .'" style="text-decoration:none;">'.
	...
	case 'slide':
		...
		foreach($images['title'] as $k => $v){
			...
			$list[$k]['summary'] = $images['summary'][$k];
	...
	/****** quicklink类型的标签 start{*******/
	case 'quicklink':
		$images = empty($label['option']['images']) ? array() : $label['option']['images'];
		$swidth = isset($label['option']['width']) ? $label['option']['width'] : 120;
		$sheight = isset($label['option']['height']) ? $label['option']['height'] : 60;
		$list = array();
		$srh[] = '<!--#p8_attach#-->';
		$srh[] = '<!--#p8_r_attach1#-->';
		...
		foreach($images['title'] as $k => $v){
			$list[$k]['title'] = $images['title'][$k];			
			$list[$k]['frame'] = empty($images['src'][$k]) ? $images['src'][$k] : '';
			if(!empty($list[$k]['frame']) && substr($list[$k]['frame'],0,8) == "<!--#p8_"){
				$list[$k]['frame'] =  attachment_url($list[$k]['frame']);
				$list[$k]['frame'] = substr($list[$k]['frame'],0,8) == "<!--#p8_" ? str_replace($srh,$rep,$list[$k]['frame']) : $list['frame'][$k];	
			}
			ob_start(); 
			eval($this->compile_template('<!--{php168}-->'. $images['url'][$k] .'<!--{/php168}-->'));
	...
	if(isset($pages)){
		//global $__label;/**fix in 20220930 会出现莫名的标签显示乱码的问题**/
		$ret['pages'] = $pages;
	}
...
function refresh_labels(){
	...
	//检查锁定
	if($lock = $this->core->CACHE->read('', 'label', 'refresh_lock', 'serialize')){
...
function add($data, $replace=false){
	...
	$data['update_time'] = P8_TIME;
	$data['last_setter'] = $USERNAME;
...
function view($id){
	...
	if($data['type'] == 'slide'){
		foreach($data['option']['images']['src'] as $key=>$val){
			$data['option']['images']['size'][$key] = $P8LANG['unknow_size'];
		}
		$query = $this->DB_master->query("SELECT * FROM $this->attach_table where `module` = 'label' and `item_id` = '$id'");
		while($row = $this->DB_master->fetch_array($query)){
			...
			$this_index = -1;
			foreach ($data['option']['images']['src'] as $index => $value) {
			...
			if($this_index >-1){
				$data['option']['images']['size'][$this_index] = $P8LANG['image_size'].$formattedSize.'M';
			}			
		}
	}
	if($data['type'] == 'image'){
		$attachment_data = $this->DB_master->fetch_one("SELECT * FROM $this->attach_table WHERE `module` = 'label' and `item_id` = '$id'");
		if($attachment_data){
			$sizeMB = $attachment_data['size'] / (1024 * 1024);
			$formattedSize = number_format($sizeMB, 2, '.', ',');
			$data['option']['image']['size'] = $P8LANG['image_size'].$formattedSize.'M';
		}else{
			$data['option']['image']['size'] = $P8LANG['unknow_size'];
		}
	}
	return $data;
}
