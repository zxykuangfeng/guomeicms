...
//P8_Label::cache($id = 0, $orig_data = array())
	...
	if($id){
		$query = $this->DB_master->query("SELECT * FROM $this->table WHERE id in ($id)");
	}else{
		$query = $this->DB_master->query("SELECT system, module,site, env, postfix FROM $this->table GROUP BY system, module, postfix, site, env");
		
		//删除上一次的缓存		
		while($arr = $this->DB_master->fetch_array($query)){
			//先备份后删除			
			cp(PHP168_PATH.'data/label/#/'.$arr['system'].'/'.$arr['module'].'/'. $arr['module'] .'_'. $arr['site'] . $arr['env'] .$arr['postfix'] . $last_cache .'.php',
				PHP168_PATH.'data/label/'.$last_cache.'/#/'.$arr['system'].'/'.$arr['module'].'/'. $arr['module'] .'_'. $arr['site'] . $arr['env'] .$arr['postfix'] . $last_cache .'.php'
			);
			cp(PHP168_PATH.'data/label/data/'.$arr['system'].'/'.$arr['module'].'/'. $arr['module'] .'_'. $arr['site'] . $arr['env'] .$arr['postfix'] . $last_cache .'.php',
				PHP168_PATH.'data/label/'.$last_cache.'/data/'.$arr['system'].'/'.$arr['module'].'/'. $arr['module'] .'_'. $arr['site'] . $arr['env'] .$arr['postfix'] . $last_cache .'.php'
			);
		...
		//删除不要的备份的文件
		$open_label_dir = opendir(PHP168_PATH.'data/label/');		
		while($dir_new = readdir($open_label_dir)){			
			if($last_cache != $dir_new && strpos($dir_new, '@') !== false && is_dir(PHP168_PATH.'data/label/'.$dir_new)){
				rm(CACHE_PATH .'label/'.$dir_new.'/');
			}
		}
	}
