...
//P8_Member::register($username, $password, $email, $datas = array(),$is_admin){
	
	// 使用正则表达式匹配空格、单引号、双引号和分号
	$username = preg_replace('/[\'"; ]/', '', $username); 
	//如果有id,则说明是从本类的方法调用过来的,其他要调用的注意,要把id清掉
	if(($inte = &$this->core->integrate()) && empty($datas['id']) && !$is_admin){
		...
		//插入本系统
		$data['role_id'] = empty($datas['role_id']) ? intval($this->core->CONFIG['member_role']) : $datas['role_id'];
		$this->core->get_cache('role');
		//角色组
		$data['role_gid'] = intval($this->core->roles[$data['role_id']]['gid']);
		...
		$data = array(
			'username' => $username,
			'password' => sha512(sha256($_password) . $salt),
			'email' => $email,
			'role_id' => empty($datas['role_id']) ? intval($this->core->CONFIG['member_role']) : $datas['role_id'],
			'role_gid' => empty($datas['role_gid']) ? intval($this->core->CONFIG['person_role_group']) : $datas['role_gid'],
		...
		if(!empty($datas['dept']))$data['dept'] = $datas['dept'];
		if(!empty($datas['dept2']))$data['dept2'] = $datas['dept2'];
		if(!empty($datas['find_pwd']))$data['find_pwd'] = $datas['find_pwd'];
