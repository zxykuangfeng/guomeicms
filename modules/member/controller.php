...
**/
function register(&$POST,$is_admin = false){
	$data = array();
	...
	//检查邮件
	if(!empty($data['email']) && ($status = $this->check_email($data['email'])) != 0){
		switch(intval($status)){
	...
	return $this->model->register($data['username'], $data['password'], $data['email'], $_data,$is_admin);
}
...
function valid_data(&$POST){
	...
	isset($POST['dept']) && $data['dept'] = html_entities($POST['dept']);
	isset($POST['dept2']) && $data['dept2'] = html_entities($POST['dept2']);
	...
	//找回密码
	if(isset($POST['find_pwd']) && $POST['find_pwd']){
		$data['find_pwd'] = $this->DB_master->escape_string(serialize($POST['find_pwd']));		
	}
	...
	if(!$usercenter && ($IS_FOUNDER || defined('ADMIN_FORCE') || defined('P8_CLUSTER'))){
		$data['is_email_manager'] = empty($POST['is_email_manager']) ? 0 : 1;
		$data['is_admin'] = empty($POST['is_admin']) ? 0 : 1;
		$data['is_founder'] = empty($POST['is_founder']) ? 0 : 1;
	}
	$data['is_message_user'] = empty($POST['is_message_user']) ? 0 : 1;
