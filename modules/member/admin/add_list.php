...
if(REQUEST_METHOD == 'GET'){
	...
	if (strtolower(file_ext($_FILES['csv_file']['name'])) != 'csv' && $_FILES["csv_file"]["error"] > 0 || "text/csv" != $_FILES["csv_file"]["type"] || ($_FILES["csv_file"]["size"] / 1024)>10000){
		message('error');
	}
	/*
	...
	$file = clean_path(clear_special_char(p8_filter_special_chars($_FILES["csv_file"]["tmp_name"])));
	$file = escapeshellarg(escapeshellcmd($file));
