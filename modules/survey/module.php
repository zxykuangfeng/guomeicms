...
function get_addon_data($iid,$id){
	$query = $this->DB_master->query("SELECT * FROM {$this->addon_table} WHERE iid = '$iid' and did='$id'");
	$addon = array();
	
	while($row = $this->DB_master->fetch_array($query)){
		$addon[$row['tid']] = $row['data'];
	}
	return $addon;
}
...
function cache_result($iid){
	...
	$query = $this->DB_master->query("SELECT iid,COUNT(*) AS `count` FROM $this->data_table $where GROUP BY iid");
...
function cache_count($iid = array()){
	$iid = is_array($iid) ? $iid : (array)$iid;
	$where = $iid ? "WHERE iid IN (".implode(',', $iid).")" : '';
	$query = $this->DB_master->query("SELECT iid,COUNT(*) AS `count` FROM $this->data_table $where GROUP BY iid");
	while($arr = $this->DB_master->fetch_array($query)){
		$this->DB_master->update(
			$this->table,
