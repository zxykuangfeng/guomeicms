...
function valid_item(&$POST){
	...
	$data['login'] = empty($POST['login']) ? 0 : 1;
	$data['cookie'] = empty($POST['cookie']) ? 0 : 1;
	$data['view_result'] = empty($POST['view_result']) ? 0 : 1;
...
function valid_title(&$POST){
	...
	$data_parm = isset($POST['data_parm']) ? (array)$POST['data_parm'] : array();
	$data['data'] = array();
	foreach($data_key as $k => $v){
		if(empty($data_value[$k])) continue;
		$data['data'][html_entities($v)] = array(html_entities($data_value[$k]),intval($data_parm[$k]));
	}
...
function delete_data($id,$iid = 0){
	
	$this->model->delete_data($id);
	if($iid) $this->model->cache_count($iid);
	return $id;
}
}
