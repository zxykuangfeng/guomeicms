...
function list_sort(){
	...
	return $this->core->list_item($select);
}
...
function label(&$LABEL, &$label, &$var){
	...
	$option = &$label['option'];
	$fids = isset($option['display_content']) ? (is_array($option['display_content']) ? $option['display_content'] : explode(',', $option['display_content'])):array();
	$select = select();
