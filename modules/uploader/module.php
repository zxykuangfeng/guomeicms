...
function upload($data,$store_path=''){
	...
	if(!empty($this->CONFIG['ftp_config']['enabled'])){
        ...
        $remote = $remote_att['current'];
		$att_url = $remote_att['server'][$remote];
	...
	foreach($data['files'] as $v){
		//唯一的文件
		$upload_file = $path .'/'. unique_id(16) .($v['ext'] ? '.'.$v['ext'] : '');
		$thumb = 0;
		list($rwidth, $rheight, $rtype, $rattr) = getimagesize($v['tmp_name']);
		$_ext = strtolower($v['ext']);
		
		if(
			empty($store_path) && (!empty($this->CONFIG['thumb']['enabled']) || !empty($this->CONFIG['watermark']['enabled']))
		){
		...
		$system_info['table_prefix'] = str_replace('P8_TABLE_','p8_',$system_info['table_prefix']);
		//有修改文件名的
		...
		if(
			...
			$id = $this->DB_master->insert(
				...
				array(
					'module' => $store_path ? $store_path : $this->_module,
					'uid' => $UID,
					'filename' => $v['alias'],
					'type' => $v['type'],
					'ext' => $_ext,
...
function delete($data,$delete_files = false){
     ...
     * @return bool
     */
function upload_chunk(&$data,$store_path=''){
    ...
    $fname  = md5($data['files']['alias']);
    $upload_file = $path . '/' . $fname . '_part'.$data['chunk'];

    $status = $func($data['files']['tmp_name'], $this->attachment_path . $upload_file);
    @chmod($this->attachment_path . $upload_file, 0644);

    if($data['chunks'] == $data['chunk'] + 1){
        sleep(5);
        $tmp_file = $this->attachment_path . $path . '/' . $fname . '.tmp';

        $out = @fopen($tmp_file, 'ab');
        for($i=0; $i<$data['chunks']; $i++){
            $source_path = $this->attachment_path . $path . '/' . $fname . '_part'.$i;
        ...
        $data['files']['tmp_name']=$tmp_file;
        $data['files']['size']=filesize($tmp_file);
        $data['files']['name']=$data['files']['alias'];
        $data['capture'] =1;
        return true;
    }
