...
if(REQUEST_METHOD == 'GET'){
	...
	$url = p8_filter_special_chars($url);
	$url or message('no_url_input');
	...
	$thumb_width = isset($_GET['thumb_width'])? intval(xss_clear($_GET['thumb_width'])):328;
	$thumb_height = isset($_GET['thumb_height'])? intval(xss_clear($_GET['thumb_height'])):247;
	$_path = str_replace($core->attachment_url, '', $url);
	if($core->CONFIG['static_enable']) $_path = str_replace($core->CONFIG['static_url'].'/attachment', '', $_path);
	...
	$_path = clear_special_char(str_replace($core->attachment_url, '', $url),true);
    $att_path = PHP168_PATH . clear_special_char($core->CONFIG['attachment']['path']) .'/';
	$_path = real_path($att_path . $_path);
    if(stristr($_path, $att_path) === false){
        exit('Hack!');
    }
