...
function valid_data(&$POST){
	...
	$data['url'] = isset($POST['url']) ? xss_url($POST['url']) : '';
	$data['forward'] = isset($POST['forward']) ? xss_url($POST['forward']) : '';
	$data['role_template'] = isset($POST['role_template']) ? $POST['role_template'] : 'center';
...
function valid_group_field_data(&$POST){
	...
	$data['is_unique'] = empty($POST['is_unique']) ? 0 : 1;
