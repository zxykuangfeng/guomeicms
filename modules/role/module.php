...
function __construct(&$system, $name){
	...
	$this->widgets = array(
		...
		'uploader'		=> 'role_model_widget_uploader',
		'multi_uploader'=> 'role_model_widget_multi_uploader',
		'image_uploader'=> 'role_model_widget_image_uploader',
		'city' 			=> 'role_model_widget_city',
	);
}
...
function cache_role($postfix=''){
	...
	foreach($this->roles as $v){
		...
		!$postfix && $this->set_menu_privilege_sites($v['id']);
	}
...
function cache_group(){
	...
	while($arr = $this->DB_master->fetch_array($query)){
		//数据表不存在则尝试修复
		$table = $this->TABLE_."group_".$arr['id'].'_data';
		$ret = $this->DB_master->checkTableExists($table);
		if(empty($ret)){		
			$sql = get_install_sql(
				$this->DB_master,
				file_get_contents($this->path .'install/group_sql.php'),
				$this->TABLE_ .'group_'. $arr['id'] .'_',
				true
			);			
			foreach($sql as $v){
...
function add_group_field(&$data){
	...
	if(
		...
		$status = $this->DB_master->query("ALTER TABLE {$this->TABLE_}group_{$data['gid']}_data ADD `$fieldname` $field");
		if($is_unique) $this->DB_master->query("ALTER TABLE {$this->TABLE_}group_{$data['gid']}_data ADD UNIQUE ($fieldname)");
		return $status;
	}
...
function update_group_field($id, &$data){
	...
	$is_unique = isset($data['is_unique']) && $data['is_unique'] ? 1 : 0;
	$fieldname = $data['name'];
	$olddata = $this->DB_master->fetch_one("SELECT * FROM $this->group_field_table WHERE id = '$id'");
	if(
		...
		$status = $this->DB_master->query("ALTER TABLE {$this->TABLE_}group_{$gid}_data CHANGE `$fieldname` `$fieldname` $field");
		if($is_unique){			
			if(!isset($olddata['is_unique']) || $olddata['is_unique'] != $is_unique){
				$unique_status = $this->DB_master->query("ALTER TABLE {$this->TABLE_}group_{$gid}_data ADD UNIQUE ($fieldname)");
				if(!$unique_status){
					$data['is_unique'] = 0;
					$this->DB_master->update(
						$this->group_field_table,
			...
			if($olddata['is_unique']){
				$this->DB_master->query("ALTER TABLE {$this->TABLE_}group_{$gid}_data DROP INDEX $fieldname");
			}
		}
		return $status;
	}
...
function set_acl($system, $module, $role_id, $acl, $info, $postfix = ''){
	...
	$status = $this->DB_master->insert(
		...
		array(
			'replace' => true,
			'unique' => array('system','module','postfix','role_id')
		)
	);
...
function set_menu_privilege_sites($role_id){
    if(isset($core->systems['sites'])) {
        $sites = $this->core->load_system('sites');
        $sitess = $sites->sites;
        foreach ($sitess as $alias=>$_){
            $this->set_menu_privilege($role_id, $alias);
        }
    }
}
...
function set_menu_privilege($role_id, $postfix=''){
	...
	$postfix_where = $postfix?" AND postfix = '$postfix'":" AND postfix = ''";
	$query = $this->DB_master->query("SELECT * FROM $this->acl_table WHERE role_id = '$role_id' $postfix_where");
