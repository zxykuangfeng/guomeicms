...
$config = $core->get_config('auditflow', '');
if(REQUEST_METHOD == 'GET'){
   ...
   // $verified= intval($_GET['verified']);
    ...
    $audomap = [];
    $formmods = $this_module->get_models();
    foreach ($formmods as $kname=>$ffd){
        $Cate = $this_module->get_model($ffd['id'], true);		
        $config = unserialize($Cate['config']);
		$audomap[$ffd['id']] = intval($core->CONFIG['audit_flow_enable_forms_'.$kname])>0?intval($config['auditflow']):0;
    }

	if($mid) $select -> in('i.mid',$mids);
    ...
    switch ($verifyfor){
        ...
        break;
        case 'auditflow':
            $select -> in('i.verified',[1,99,88,-99],true);
            $mids= [];
            foreach ($audomap as $_mid=>$_mvd){
                $_mvd && $mids[]=$_mid;
            }
            $select -> in('i.mid',$mids);
            break;
        case 'unchecked':
            $select -> in('i.verified',[99,-99]);
            break;
        case 'recycled':
            $select -> in('i.verified',88);
            break;
        default:
            $verifyfor = 'all';
    }
   // $conditon = $auditFlow->getCondition('forms','mainstation', $UID, false, 'mid');
   // echo "condition:$conditon\n";
  //  $select->where($conditon);
	...
	foreach($list as $key=>$detail){
        ...
        $list[$key]['auditflow'] = $audomap[$detail['mid']];
	...
	include template($this_module, 'manage_auditflow');
