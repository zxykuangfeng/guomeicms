	...
	foreach($models as $model){
		...
		foreach($fields as $field){
			...
			foreach($field_data as $key=>$val){
				...
				if(is_array($val)){
					$vals = stripslashes(html_decode_entities($val['0']));
					$note = stripslashes(html_decode_entities($val['1']));
					$field['data'][$key] = array($vals,$note);
				}else{
			...
			$field['CONFIG']['print'] = !isset($field['CONFIG']['print']) || !empty($field['CONFIG']['print']) ? 1 : 0;
