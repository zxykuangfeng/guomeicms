...
function get_category($reflash=false){
	...
	if($reflash || !$return = $CACHE->read($SYSTEM .'/modules/', $MODULE, 'cate','serialize')){
		$query = $this->DB_master->fetch_all("SELECT * FROM $this->table_cat ORDER BY display_order desc,num ASC");		
...
function updateCat($data,$type){
	if(!empty($data['new'])){
		foreach($data['new'] as $key=>$name){
			...
			$this->DB_master->insert($this->table_cat, $this->DB_master->escape_string(array('type'=>$type,'name'=>$name,'num'=>$num['num']+1)));
		}
	}
	if(!empty($data['old'])){
		$order = array_flip($data['order']);
		foreach($data['old'] as $id=>$name){
			$this->DB_master->update($this->table_cat, $this->DB_master->escape_string(array('type'=>$type,'name'=>$name,'num'=>$order[$id])),"id='$id'");
		}
	}
}
	...
	foreach($data as $key=>$item){
		foreach($item['id'] as $k=>$id){
			...
			if($id){
				...
				//新增				
				$num = $this->DB_master->fetch_one("SELECT num FROM {$this->table_cat} WHERE type='1' ORDER BY num DESC LIMIT 1");
				$data['num'] = $num['num']+1;
				$this->DB_master->insert($this->table_cat, $this->DB_master->escape_string($data));
			}			
		}
	}
}
...
function comment($id,$common,$snumber=''){
	$where = $snumber ? "id='$id'" : "id='$id' and code='$snumber'";
...
function getstatistics3(){
	$sql = "SELECT `type`,count(*) amount FROM {$this->table} GROUP BY `type` desc";
	$query_amount = $this->DB_master->fetch_all($sql);
	$sql2 = "SELECT `type`,count(*) done FROM {$this->table} where `status`>0 GROUP BY `type`";
	$query_done = $this->DB_master->fetch_all($sql2);
	foreach($query_amount as $item){
		$data[$item['type']]['type'] = $item['type'];
		$data[$item['type']]['amount'] = $item['amount'];
	}
	foreach($query_done as $item){
		$data[$item['type']]['done'] = $item['done'];
	}
	return $data;
}
...
function getdp($row){
	$dp = 1;
    if($row['status_change_time'])$row['create_time']=$row['status_change_time'];
...
function label(&$LABEL, &$label, &$var){
	...
	if(empty($option['statistic_id'])){
		...
		if(empty($option['ids'])){
            ...
            if($this->CONFIG['display_model'])
				$select -> in('i.undisplay',array(0,1));
			else
				$select -> in('i.undisplay',0);
			//$select -> in('vefify',1);
			$select -> in('i.visual',1);
			...
			//推荐
			if(!empty($option['recommend'])) $select->in("i.recommend",1);
...
function get_total($filter=array()){
	...
	if(!empty($filter['begin_time'])){
		$begin_time = $filter['begin_time']=='this month' ? mktime(0,0,0,date('m'),1,date('Y')) : strtotime($filter['begin_time']);
...
function tonji_01($filter=array()){
    ...
    if(!empty($filter['begin_time'])){
		$begin_time = $filter['begin_time']=='this month' ? mktime(0,0,0,date('m'),1,date('Y')) : strtotime($filter['begin_time']);
		$select->where("i.create_time>='".$begin_time."'");
	}
...
//办理总数abs
function tonji_03($filter=array(),$flag = false){
	$select = select();
	$select->from($this->table .' AS i', 'department, COUNT(1) as `count`');
	$select->group('i.department');
	$select->order('count DESC');
...
//全部红灯
function tonji_05($filter=array(),$flag = false){
	$select = select();
	$select->from($this->table .' AS i', 'department, COUNT(1) as `count`');
	$select->group('i.department');
	$select->order('count DESC');
	...
	return $flag ? array('list'=>$temp,'pages'=>$pages) : array('list'=>$list,'pages'=>$pages);
}
//全部黄灯
...
function cacheManager(){
	...
	while($row = $this->DB_master->fetch_array($query)){
		...
		$manager = isset($vd['my_letter_manage']['manager']) ? $vd['my_letter_manage']['manager'] : array();
...
function sendMsg($id){
	...
	while($user_data = $this->DB_master->fetch_array($query)){
		...
		if(in_array('email',$type) && $semail){			
			...
			//没开启时不发送
			if(isset($email->CONFIG['enable']) && empty($email->CONFIG['enable'])){
