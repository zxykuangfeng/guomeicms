...
if(REQUEST_METHOD == 'GET'){
	...
	array_multisort(array_column($options,'display_order'),SORT_DESC,$options);
	...
	//时间间隔内重复投票
	if(empty($data['enabled_24']) && get_cookie('vote_post_'.$id)){
        message($P8LANG['vote_24hour_interval']);
    }
	...
	if($data['roles'] && !isset($data['roles'][$ROLE])){
		if(!$UID)
			message('role_not_allow_vote',$core->url.'/index.php/member-register',1);
		else
    ...
    $ss = set_cookie('vote_post_'.$id,time(),86400);
