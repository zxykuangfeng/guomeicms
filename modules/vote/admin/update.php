...
if(REQUEST_METHOD == 'GET'){
	...
	$option_votes = isset($_POST['option_votes']) ? (array)$_POST['option_votes'] : array();
	...
	while($arr = $DB_master->fetch_array($query)){
		...
		if(isset($option_name[$arr['id']])){
			$DB_master->update(
				...
				array(
					...
					'votes' => empty($option_votes[$arr['id']]) ? 0 : intval($option_votes[$arr['id']]),
		...
		$votes += intval($option_votes[$arr['id']]);
	}
	...
	foreach($option_name as $k => $v){
		$oid = $DB_master->insert(
			...
			array(
				...
				'votes' => isset($option_votes[$k]) ? intval($option_votes[$k]) : 0,
