...
function cache(){
	$interval_day = isset($this->CONFIG['interval_day']) && $this->CONFIG['interval_day'] ? intval($this->CONFIG['interval_day']) : 0;
	if($interval_day){
		$this->DB_master->delete(
			$this->table,
			"(sender_uid = '0' or username = 'system') and `type` in ('in','important') and `timestamp` < UNIX_TIMESTAMP(DATE_SUB(NOW(), INTERVAL $interval_day DAY))"
		);
	}
}
