...
if(REQUEST_METHOD == 'GET'){
    ...
    switch($action){
        ...
        case 'download':
            ...
            if(!is_dir(CACHE_PATH .'db_backup/'. $name)){
                exit(json_encode(['code'=>0,'msg'=>'file not exist']));
            }


            $att_path = CACHE_PATH .'db_backup/'. $name .'/';
            $_path = md5($name).'.zip';;
            $_path = $att_path . $_path;


            $zipname = md5($name).'.zip';

            include PHP168_PATH.'/inc/pclzip.lib.php';
            $pclzip = new PclZip($_path);

            $flsi = scandir(CACHE_PATH .'db_backup/'.$name);
            ...
            $pclzip->create($fff,PCLZIP_OPT_REMOVE_ALL_PATH);
