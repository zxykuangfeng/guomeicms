...
<input type="hidden" name="ours" value="1">
</form>
...
if(empty($_POST['tid'])){
	...
	if($CACHE->read($SYSTEM .'/modules/', $MODULE, 'backup_lock', 'serialize')){
		message('dbm_backup_locked','',3,$core->admin_controller);
	}
	...
	if(isset($_POST['name']) && $_POST['name']){
		foreach($tables as $key=>$val){
			if(!in_array($val['Name'],$_POST['name'])){
				unset($tables[$key]);
			}
		}
		if(count($_POST['name']) != count($tables)){
			foreach($_POST['name'] as $val){
				if(!in_array($val,$tables)){
					$tables[] = array('Name' => $val);
				}
			}			
		}
	}
	...
	$task = array(
		...
		'charset' => $charset,
		'path' => 'db_backup/'. date('Y-m-d#H_i', P8_TIME).$afx. '_'. $charset,
		'tables' => array()
	);
