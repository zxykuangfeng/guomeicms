...
class cmscall
{
    ...
    public function getitem()
    {
        ...
        foreach ($steps as $step => $verifi) {
            ...
            if (!$IS_FOUNDER && !$this_module->checkAudit($data['cid'], $UID, $verifi, self::CALL_MODULE, 'mainstation')) {
                break;
            }
        ...
        if ($IS_FOUNDER
            ...
            or (1 == $_verified && $this_module->checkAudit($data['cid'],$UID, $finalVerifi[0], self::CALL_MODULE))
            or $this_module->checkPreLast($id,$USERNAME, self::CALL_MODULE)
            or ($verified==-1 && $this_module->checkNextAudit($data['cid'], $UID, $verified, self::CALL_MODULE))
        ) {
