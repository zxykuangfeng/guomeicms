...
if(REQUEST_METHOD == 'GET'){
	...
	$env_items = array(
		...
		'mysql' => array('need' => '5.0', 'best' => '5.5'),		
		'attachmentupload' => array('need' => 'notset', 'best' => '2M'),
	...
	foreach($env_items as $key => $item) {
		if($key == 'php') {
			...
			if(($tmp = round($size / pow(1024, 3))) > 1)
				$env_items[$key]['current'] = $tmp.' GB';
			elseif(($tmp = round($size / pow(1024, 2))) > 0)
				$env_items[$key]['current'] = $tmp.' MB';
			elseif(($tmp = round($size / pow(1024, 1))) > 0)
				$env_items[$key]['current'] = $tmp.' KB';
			else
				$env_items[$key]['current'] = $tmp.' B';
		} elseif($key == 'mysql') {
			$env_items[$key]['current'] = $DB_master->version();
		} elseif($key == 'memcache') {
			$env_items[$key]['current'] = class_exists('Memcache') ? $P8LANG['env']['enable'] : $P8LANG['env']['disable'];
		} elseif($key == 'eAccelerator') {
		...
		} elseif($key == 'base_root') {
			$env_items[$key]['current'] = P8_ROOT;
		} elseif($key == 'php168_path') {
			$env_items[$key]['current'] = PHP168_PATH;
		} elseif($key == 'php168_ip') {
