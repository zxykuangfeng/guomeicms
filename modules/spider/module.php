...
function _capture_image($m){
	if(strpos($m[1], "http") === 0 || strpos($m[1], "https") === 0){
		//避免重复上传
		$orl = array_column($this->capture_images,'orl');
		if($orl){
			if(in_array($m[1],$orl)){
				$index = array_search($m[1],$orl);
				$capture_attachment = $this->capture_images[$index]['att'];
			}else{
				$capture_attachment = $this->capture_attachment($m[1]);
				$this->capture_images[] = array(
					'orl' => $m[1],
					'att' => $capture_attachment
				);
			}
		}else{
			$capture_attachment = $this->capture_attachment($m[1]);
		...
		return str_replace($m[1], $capture_attachment, $m[0]);
	}else{
		return $m[0];
	}
}
