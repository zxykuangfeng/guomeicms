...
function valid_data(&$POST){
	...
	$data['width'] = $data['width'] ? intval($data['width']) : '';
	$data['height'] = isset($POST['height']) ? preg_replace('[^0-9%px]', '', $POST['height']) : '';
	$data['height'] = $data['height'] ? intval($data['height']) : '';
	...
	$data['source'] = isset($POST['source']) ? trim($POST['source']) : '';
	$data['buyable'] = empty($POST['buyable']) ? 0 : 1;
	$data['manager'] = isset($POST['manager']) && !empty($POST['manager']) ? implode(',',$POST['manager']) : '';
...
function valid_buy(&$POST){
	...
	if(!empty($POST['id']))$data['id'] = intval($POST['id']);
...
function valid_url($data){
	if(is_array($data)){
		...
		if(!preg_match('#^(http|https)://.+$#i', $data)){
			if(!preg_match('#^/attachment/.+$#i', $data)){
				return '';
			}
		}		
