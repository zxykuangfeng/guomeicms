...
function __construct(&$system, $name){
	...
	$this->widgets = array(
		...
		'video_multi_uploader'=> 'cms_model_widget_video_multi_uploader',
		...
		'vscode'		=> 'cms_model_widget_vscode',
...
function export($name, $new_name = ''){
	...
	//所有导出的文件转回gbk
	//require_once PHP168_PATH .'inc/install.func.php';
	//if($this->core->CONFIG['page_charset'] != 'utf-8')
	//	convert_file_encode($path, $this->core->CONFIG['page_charset'], 'utf-8', '.js|.css|.html|.php');
	
	//转换编码,转回GBK
	//if($this->core->CONFIG['page_charset'] != 'utf-8')
	//$data = convert_encode($this->core->CONFIG['page_charset'], 'utf-8', $data);
...
function cache($name = ''){
	...
	$models = $this->DB_master->fetch_all("SELECT * FROM $this->table order by list_order desc");
...
function edit_template_callback($m){
	...
	foreach($this->_model_fields as $v){
		...
		case 'checkbox': case 'multi_select': case 'video_multi_uploader':
			$__name = '[]';
		break;
		case 'uploader':
			$__name = '[url]';
