...
//P8_CMS_Category::cache($cache_all = true, $list_cache = true, $ids = array())
	...
	$list = $this->DB_master->query("SELECT * FROM $this->table_recycle ORDER BY display_order DESC");
	...
	foreach($this->categories_recycle as $v){
		...
		$this->categories_recycle[$v['id']]['enable_show'] = isset($this->categories_recycle[$v['id']]['CONFIG']['enable_show']) && !empty($this->categories_recycle[$v['id']]['CONFIG']['enable_show']) ? 1 : 0;
		if($v['parent']){
			...
			if(empty($this->categories_recycle[$v['parent']])){
				...
				$this->top_categories_recycle[$v['id']] = &$this->categories_recycle[$v['id']];
		...
		if(empty($this->categories_recycle[$v['id']]['url'])){
			...
			$this->categories_recycle[$v['id']]['url'] = p8_url($item, $this->categories_recycle[$v['id']], 'list');
	...
	if($list_cache){
		$this->core->CACHE->write($this->system->name .'/modules/', $this->name, 'categories_recycle', $this->data, 'serialize');
		
		//$json = array('json' => p8_json($this->make_json_sort($_top_categories)));
		//$json['path'] = p8_json($path);
		//$this->core->CACHE->write($this->system->name .'/modules/', $this->name, 'json_recycle', $json);
