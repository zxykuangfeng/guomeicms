...
//P8_CMS_Category::cache($cache_all = true, $list_cache = true, $ids = array())
	...
	while($arr = $this->DB_master->fetch_array($list)){
		...
		$arr['frame'] && $arr['frame'] = attachment_url($arr['frame'],false,true);
	...
	foreach($this->categories as $v){
		...
		$this->categories[$v['id']]['enable_show'] = isset($this->categories[$v['id']]['CONFIG']['enable_show']) && !empty($this->categories[$v['id']]['CONFIG']['enable_show']) ? 1 : 0;
		...
		$this->categories[$v['id']]['allow_ip_enabled'] = isset($this->categories[$v['id']]['CONFIG']['allow_ip']['enabled']) ? (int)$this->categories[$v['id']]['CONFIG']['allow_ip']['enabled'] : 0;
		$this->categories[$v['id']]['attachment_type'] = !empty($this->categories[$v['id']]['CONFIG']['attachment_type']) ? 1 : 0;
		...
		if(empty($this->categories[$v['id']]['url'])){
			...
			if(!empty($this->categories[$v['id']]['CONFIG']['direct_to_category_id'])){
				$direct_to_category_id = intval($this->categories[$v['id']]['CONFIG']['direct_to_category_id']);
				if($this->categories[$direct_to_category_id]['CONFIG']['allow_ip']['enabled']) $this->categories[$direct_to_category_id]['htmlize'] = 0;
				$this->categories[$v['id']]['url'] = p8_url($item, $this->categories[$direct_to_category_id], 'list');
				$this->categories[$v['id']]['m_url'] = p8_url($item, $this->categories[$direct_to_category_id], 'list',true,true);
			}else{
				if(isset($this->categories[$v['id']]['CONFIG']['allow_ip']['enabled']) && $this->categories[$v['id']]['CONFIG']['allow_ip']['enabled']) $this->categories[$v['id']]['htmlize'] = 0;
				$this->categories[$v['id']]['url'] = p8_url($item, $this->categories[$v['id']], 'list');
				$this->categories[$v['id']]['m_url'] = p8_url($item, $this->categories[$v['id']], 'list',true,true);				
			}
	...
	//修复栏目内容数
	/*
	$this->DB_master->update($this->table, array('item_count' => 0), '');
	$item_config = $this->core->get_config($this_system->name,'item');		
	...
	if($lan_date_enable && $lan_date_timestamp){
		...
		$query = $this->DB_master->query("SELECT cid, COUNT(*) AS `count` FROM $item_table where $where GROUP BY cid");
		while($arr = $this->DB_master->fetch_array($query)){
			$this->update_count($arr['cid'], $arr['count']);
		}
		//不限制部分
		if(!empty($s)){
			$query = $this->DB_master->query("SELECT cid, COUNT(*) AS `count` FROM $item_table where `cid` IN ($s) GROUP BY cid");
			while($arr = $this->DB_master->fetch_array($query)){
				$this->update_count($arr['cid'], $arr['count']);
			}
		}
	}else{
		$query = $this->DB_master->query("SELECT cid, COUNT(*) AS `count` FROM $item_table GROUP BY cid");
		while($arr = $this->DB_master->fetch_array($query)){
			$this->update_count($arr['cid'], $arr['count']);
		}
	}
