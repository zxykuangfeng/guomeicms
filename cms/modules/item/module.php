...
function set_content_html($cid = array(),$limit = false){
	...
	$lan_category = isset($this->CONFIG['lan_category']) && $this->CONFIG['lan_category'] ? explode(',',$this->CONFIG['lan_category']) : array();
	...
	foreach($all_models as $model=>$val){		
		...
		$query = $this->DB_slave->query("select `id`,`cid`,`pages`,`timestamp`,`config` from $table $where");
		while($arr = $this->DB_slave->fetch_array($query)){
			...
			//获取要删除的HTML文件
			$category = $this->system->fetch_category($arr['cid']);
			if(!empty($category['htmlize'])){				
				$arr['#category'] = $category;				
				if($file = p8_html_url($this, $arr, 'view', false)){
...
function data($act, $data){
	...
	if($act == 'read'){
		$sql = 'SELECT id,uid,verified,model,views,cid,pages,allow_comment,credit,credit_type,html_view_url_rule,url,`timestamp`,verify_frame FROM '. $this->main_table .' WHERE id = \''. $data .'\'';
		//读取
...
**/
function uploaded_attachments($item_id,$common_attr){
	global $UID, $core;
	if(!$UID || !$common_attr) return false;		
	//只能修改自己上传的附件,如果UID为0,限制IP
	$data = $core->DB_master->fetch_array("SELECT * FROM $core->attachment_table WHERE module = 'item' AND uid = '$UID'");
	//var_dump($data);
	unset($data['id']);
	$data['module'] = $common = $obj->system->name.'_common';
	$core->DB_master->update(
		$core->attachment_table,
		$data,
		"item_id = $item_id AND module = '$common' AND uid = '$UID'". (empty($UID) ? ' AND ip = \''. P8_IP .'\'' : '')
	);	
	return true;
}
/**
...
function check_repush($datas){
	...
	foreach($datas as $key => $data){
		...
		$cid = $data['cid'];
		$iid = $data['iid'];
		$where = "`from` = 'cms' and `to` =  'sites' and `sc` = 's' and `status` in ('0','1','2')";
...
function label(&$LABEL, &$label, &$var){
	...
	if(!empty($model)){
		...
		$fields = 'i.id,i.authority, i.model, i.title, i.title_color, i.title_bold, i.sub_title, i.cid, i.frame, i.url, i.uid, i.username, i.attributes, i.summary, i.html_view_url_rule, i.views, i.level, i.source, i.author, i.comments, i.timestamp, i.list_order';
	}
	...
	if(empty($option['ids'])){
		...
		if(!empty($option['attribute'])){
			...
			intval($option['attribute']) == 9 ? $select->in('a.aid', 5,true) : $select->in('a.aid', $option['attribute']);
			$sphinx['enabled'] = 0;
		...
		if(!empty($var['role'])){
			$v = intval($var['role']);
			$where = "(authority = ''";
			$where .= " or find_in_set($v,authority)";
			$where .= ")";
			$select->where($where);
		}else{
			if(isset($option['authority']) && !empty($option['authority'])){
				$authority = explode(',',$option['authority']);
				$where = "(authority = ''";
		...
		//针对排序
		if(array_key_exists('i.views_7',$option['order_by'])){
			$select->range('i.timestamp', strtotime("-7 day"));
		}elseif(array_key_exists('i.views_30',$option['order_by'])){	
			$select->range('i.timestamp', strtotime("-30 day"));
		}elseif(array_key_exists('i.views_90',$option['order_by'])){
			$select->range('i.timestamp', strtotime("-90 day"));
		}
		//排序
	...
	foreach($list as $k => $v){
		...
		$list[$k]['url'] = $v['#category']['allow_ip_enabled'] >= 1 || $v_config['allow_ip']['enabled'] >= 1 ? $main_domain.'/index.php/cms/item-view-id-'.$v['id'].'.html' : $list[$k]['url'];
		//权限控制下使用动态
