...
if(empty($_POST['task'])){
	...
	//if(empty($time_range)){
	//	$time_range[0] = date('Y-m-d H:i:s',strtotime('-7 day'));
	//	$time_range[1] = date('Y-m-d H:i:s',strtotime('now'));
	//}
	//ID范围
	...
	//echo $cond;exit;
	//echo "SELECT COUNT(*) AS num FROM $this_module->main_table $cond";
	...
	for($i = 0; $i < $threads; $i++){
		...
		$iframe .= p8lang($P8LANG['htmlize2']['thread'], array($i)) .
...
if(!$this_module->html($query)){
	...
	if(!$this_module->html($query)){
		echo p8lang($P8LANG['cms_item_html_unwritable'], array($this_module->_html['file']));
...
if($__page__ >= $__thread__['pages']){
	...
	echo p8lang($P8LANG['htmlize2']['done'], array(P8_TIME - $__task__['time']));
	...
	_poster(
		$__task_name__,
		$_POST['thread_id'],
		p8lang($P8LANG['htmlize2']['view_process'], $__thread__['page'], $__thread__['pages'])
	);
}
//##线程结束}
