...
if(REQUEST_METHOD == 'GET'){
	...
	$allow_comment = $this_controller->check_action('post_comment');

	//查询内容
	$select = select();
	$select->from($this_module->main_table, 'id, model, cid, url, title, timestamp, views,level, comments, allow_comment');
	$select->in('id', $iid);
	...
	if(!P8_AJAX_REQUEST){
		...
		$item_tpl = $this_module->CONFIG['order']['enabled'] ? 'comment_order' : 'comment';
		include template($this_module, $item_tpl);
		...
		//评论列表
		$select = select();
		$select->from($this_module->TABLE_ .'comment', 'id,ip, username, quote, content, timestamp, digg, oppose');
		$select->in('iid', $iid);
		...
		if($ids){
			$select = select();
			$select->from($this_module->TABLE_ .'comment', 'id,ip, username, content, timestamp');
	...
	switch($action){
		case 'add':
			...
			//检查权限
			$this_controller->check_action('post_comment') or exit('<script type="text/javascript">alert("'. $P8LANG['no_privilege'].'"); window.parent.change_count(); </script>');
			...
			if(empty($check) || empty($check['allow_comment'])) exit('<script type="text/javascript">alert("'. $P8LANG['cms_item']['comment_denied'].'"); window.parent.change_count(); </script>');
			...
			//编号
			$vscode = $this_module->CONFIG['order']['code'] ? $this_module->CONFIG['order']['code'] : 'dx_';
			$data['code'] = $vscode.date('Ymd', P8_TIME).mt_rand(9999,99999);
			//自定义
			$data['field_1'] = isset($_POST['field_1']) ? filter_word(html_entities(trim($_POST['field_1']))) : '';
			$data['field_2'] = isset($_POST['field_2']) ? filter_word(html_entities(trim($_POST['field_2']))) : '';
			$data['field_3'] = isset($_POST['field_3']) ? filter_word(html_entities(trim($_POST['field_3']))) : '';
			$data['field_4'] = isset($_POST['field_4']) ? filter_word(html_entities(trim($_POST['field_4']))) : '';
			$data['field_5'] = isset($_POST['field_5']) ? filter_word(html_entities(trim($_POST['field_5']))) : '';
			$data['field_6'] = isset($_POST['field_6']) ? filter_word(html_entities(trim($_POST['field_6']))) : '';
			$data['field_7'] = isset($_POST['field_7']) ? filter_word(html_entities(trim($_POST['field_7']))) : '';
			//引用评论
			$data['quote'] = '';
