...
//P8_CMS_Item::html(&$query)
	...
	$lan_date_timestamp = isset($this->CONFIG['lan_date']) && $this->CONFIG['lan_date'] ? intval($this->CONFIG['lan_date']) : 0;
	...
	if(empty($__models__) && $ids){
		$query = $this->DB_master->query("SELECT * FROM $this->main_table WHERE id IN ($ids)");
		while($__arr__ = $this->DB_master->fetch_array($query)){
			//设置有跳转的跳过
			if(!empty($HTML_DATA['url'])) continue;
