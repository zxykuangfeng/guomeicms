...
//排序
$str = $comma = '';
if(sizeof($option['order_by'])==1 && array_key_exists('i.level',$option['order_by']))
    $option['order_by'] = array('i.level'=>'1','i.list_order'=>'1');
foreach($option['order_by'] as $field => $desc){
	...
	if($field=='i.views_7' || $field=='i.views_30' || $field=='i.views_90') $field = 'i.views';	
...
//模板规则
if(!empty($option['tplcode']) && strlen($option['tplcode']) > 10){
	if(strpos($option['tplcode'],'<!--{php168}-->') === false || strpos($option['tplcode'],'<!--{/php168}-->') === false) message('label_template_syntax_error');
}
//限制
