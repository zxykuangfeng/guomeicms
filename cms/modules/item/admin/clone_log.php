...
$this_controller->check_admin_action('clone') or message('no_privilege');
if(REQUEST_METHOD == 'GET'){
	...
	if(!P8_AJAX_REQUEST){	
		...
		include template($this_module, 'clone_log', 'admin');
		exit;
	}else{
		//JS传过来的关键字,UTF-8的
	...
	$num = $DB_master->delete($this_module->clone_table, "clone_id IN (".implode(',', $id).")");
	$ret = $num ? $id : array();
	exit(jsonencode($ret));
}