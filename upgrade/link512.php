...
$sql = "ALTER TABLE `$main_table` CHANGE `url` `url` VARCHAR( 512 ) NOT NULL DEFAULT '';";
$sql = get_install_sql($DB_master, $sql, $core->TABLE_, true);
foreach($sql as $v){
    $DB_master->query($v);
}
echo "CMS系统主数据表升级完成！！<br/>";
foreach ($list as $v){
    $table = $main_table.'_'.$v['name'].'_';
    $sql = "ALTER TABLE `$table` CHANGE `url` `url` VARCHAR( 512 ) NOT NULL DEFAULT '';";
    $sql = get_install_sql($DB_master, $sql, $core->TABLE_, true);
