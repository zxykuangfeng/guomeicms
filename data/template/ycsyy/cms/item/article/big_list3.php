...
<div class="title pull-left">$CAT[name]</div> 
...
<div class="row row30">
EOT;
$__t_foreach = @$subcategories;
if(!empty($__t_foreach)){
foreach($__t_foreach as $v){
$subcat = array($v['id']) + $category->get_children_ids($v['id']);
print <<<EOT

<div class="col-md-6 col-sm-6 col-xs-12 col-pd-30">
<div class="part-list">
<div class="list-title clearfix">
<div class="title pull-left"><a href="$v[url]">$v[name]</a></div> 
<span class="more pull-right"> 
EOT;
if($add_enable){
print <<<EOT

<a href="{$core->U_controller}/cms/item-add?cid=$v[id]&model=$CAT[model]" target="_blank">发表</a> | 
EOT;
}
print <<<EOT
...
EOT;
echo $LABEL->display('CMS子分类内容列表list3', array('i.cid' => $subcat, 'model' => $MODEL));
print <<<EOT
